...
public class Matrix4d implements Serializable, Cloneable
{
    ...
    public final void get(final Quat4f quat4f) {
        ...
        if (((n < 0.0) ? (-n) : n) >= 1.0E-30) {
            quat4f.w = (float)Math.sqrt(n);
            final double n2 = 0.25 / quat4f.w;
            quat4f.x = (float)((array[7] - array[5]) * n2);
            quat4f.y = (float)((array[2] - array[6]) * n2);
            quat4f.z = (float)((array[3] - array[1]) * n2);
            return;
        }
        quat4f.w = 0.0f;
        final double n3 = -0.5 * (array[4] + array[8]);
        if (((n3 < 0.0) ? (-n3) : n3) >= 1.0E-30) {
            quat4f.x = (float)Math.sqrt(n3);
            final double n4 = 0.5 / quat4f.x;
            quat4f.y = (float)(array[3] * n4);
            quat4f.z = (float)(array[6] * n4);
            return;
        }
        quat4f.x = 0.0f;
        final double n5 = 0.5 * (1.0 - array[8]);
        if (((n5 < 0.0) ? (-n5) : n5) >= 1.0E-30) {
            quat4f.y = (float)Math.sqrt(n5);
    ...
    public final void get(final Quat4d quat4d) {
        ...
        final double n = 0.25 * (1.0 + array[0] + array[4] + array[8]);
        if (((n < 0.0) ? (-n) : n) >= 1.0E-30) {
            quat4d.w = Math.sqrt(n);
            final double n2 = 0.25 / quat4d.w;
            quat4d.x = (array[7] - array[5]) * n2;
            quat4d.y = (array[2] - array[6]) * n2;
            quat4d.z = (array[3] - array[1]) * n2;
            return;
        }
        quat4d.w = 0.0;
        final double n3 = -0.5 * (array[4] + array[8]);
        if (((n3 < 0.0) ? (-n3) : n3) >= 1.0E-30) {
            quat4d.x = Math.sqrt(n3);
            final double n4 = 0.5 / quat4d.x;
            quat4d.y = array[3] * n4;
            quat4d.z = array[6] * n4;
            return;
        }
