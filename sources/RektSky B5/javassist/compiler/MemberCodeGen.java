...
extends CodeGen {
    ...
    protected void atFieldAssign(Expr expr, int op, ASTree left, ASTree right, boolean doDup) throws CompileError {
        ...
        if (op == 61) {
            ...
            fi = maker == null ? this.addFieldrefInfo(f2, finfo) : 0;
        } else {
        ...
        this.atFieldAssignCore(f2, is_static, fi, is2w);
        this.exprType = fType;
        this.arrayDim = fDim;
    ...
    private int atFieldRead(CtField f2, boolean isStatic) throws CompileError {
        ...
        int fi = this.addFieldrefInfo(f2, finfo);
        if (isStatic) {
