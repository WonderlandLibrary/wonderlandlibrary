...
public final class NetUtil {
    ...
    static {
        ...
        localhost = null;
        if (NetUtil.LOOPBACK_IF != null) {
            NetUtil.logger.debug("Loopback interface: {}", (Object)NetUtil.LOOPBACK_IF.getDisplayName());
            addrs = NetUtil.LOOPBACK_IF.getInetAddresses();
            while (addrs.hasMoreElements()) {
                a = addrs.nextElement();
                if (localhost == null) {
                    NetUtil.logger.debug("Loopback address: {} (primary)", (Object)a);
                    localhost = a;
                    continue;
                }
                NetUtil.logger.debug("Loopback address: {}", (Object)a);
            }
        }
        localhost4 = null;
        try {
        ...
        NetUtil.LOCALHOST = localhost;
        somaxconn = 3072;
        in = null;
        try {
            in = new BufferedReader(new FileReader("/proc/sys/net/core/somaxconn"));
            somaxconn = Integer.parseInt(in.readLine());
            NetUtil.logger.debug("/proc/sys/net/core/somaxconn: {}", (Object)somaxconn);
