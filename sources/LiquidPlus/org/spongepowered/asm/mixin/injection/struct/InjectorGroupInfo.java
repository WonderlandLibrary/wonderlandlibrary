...
public class InjectorGroupInfo {
    ...
    extends HashMap<String, InjectorGroupInfo> {
        ...
        public InjectorGroupInfo parseGroup(AnnotationNode annotation2, String defaultGroup) {
            Integer max;
            if (annotation2 == null) {
                return NO_GROUP;
            }
            String name = (String)Annotations.getValue(annotation2, "name");
            if (name == null || name.isEmpty()) {
                name = defaultGroup;
            }
            InjectorGroupInfo groupInfo = this.forName(name);
            Integer min = (Integer)Annotations.getValue(annotation2, "min");
            if (min != null && min != -1) {
                groupInfo.setMinRequired(min);
            }
            if ((max = (Integer)Annotations.getValue(annotation2, "max")) != null && max != -1) {
