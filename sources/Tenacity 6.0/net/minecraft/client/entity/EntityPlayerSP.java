...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdateWalkingPlayer() {
        ...
        if (flag != EntityPlayerSP.serverSprintState) {
            if (flag) {
            ...
            EntityPlayerSP.serverSprintState = flag;
        }
        final boolean flag2 = this.isSneaking();
        if (flag2 != EntityPlayerSP.serverSneakState) {
            if (flag2) {
            ...
            EntityPlayerSP.serverSneakState = flag2;
        }
        if (this.isCurrentViewEntity()) {
            ...
            if (!motionEvent.isCancelled()) {
                final double posX = motionEvent.getX();
                final double posY = motionEvent.getY();
                final double posZ = motionEvent.getZ();
                final float rotationYaw = motionEvent.getYaw();
                final float rotationPitch = motionEvent.getPitch();
                final boolean onGround = motionEvent.isOnGround();
                final double d0 = posX - this.lastReportedPosX;
                final double d2 = posY - this.lastReportedPosY;
                final double d3 = posZ - this.lastReportedPosZ;
                final double d4 = rotationYaw - this.lastReportedYaw;
                final double d5 = rotationPitch - this.lastReportedPitch;
                ...
                if (this.ridingEntity == null) {
                    ...
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0, this.motionZ, rotationYaw, rotationPitch, onGround));
                    flag3 = false;
                }
    ...
    public void sendChatMessage(final String message) {
        ...
        final PlayerSendMessageEvent playerSendMessageEvent = new PlayerSendMessageEvent(message);
    ...
    protected boolean pushOutOfBlocks(final double x, final double y, final double z) {
        final PushOutOfBlockEvent pushOutOfBlockEvent = new PushOutOfBlockEvent();
    ...
    public void onLivingUpdate() {
        ...
        if (this.isUsingItem() && !this.isRiding()) {
            final NoSlow noSlow = Tenacity.INSTANCE.getModuleCollection().getModule(NoSlow.class);
            ...
            if (!slowDownEvent.isCancelled()) {
                ...
                movementInput.moveStrafe *= slowDownEvent.getForwardMult();
                final MovementInput movementInput2 = this.movementInput;
                movementInput2.moveForward *= slowDownEvent.getStrafeMult();
    ...
    @Override
    public void jump() {
        final JumpEvent jumpEvent = new JumpEvent();
