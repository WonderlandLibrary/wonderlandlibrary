...
class MixinApplicatorStandard {
    ...
    void apply(SortedSet<MixinInfo> mixins) {
        ...
        try {
            ...
            Iterator iterator4 = mixinContexts.iterator();
            while (iterator4.hasNext()) {
                MixinTargetContext context;
                current = context = (MixinTargetContext)iterator4.next();
    ...
    protected void checkMethodConstraints(MixinTargetContext mixin, MethodNode method) {
        for (Class<? extends Annotation> annotationType : CONSTRAINED_ANNOTATIONS) {
            AnnotationNode annotation2 = Annotations.getVisible(method, annotationType);
            if (annotation2 == null) continue;
            this.checkConstraints(mixin, method, annotation2);
        }
    }

    protected final void checkConstraints(MixinTargetContext mixin, MethodNode method, AnnotationNode annotation2) {
        try {
            ConstraintParser.Constraint constraint = ConstraintParser.parse(annotation2);
            try {
