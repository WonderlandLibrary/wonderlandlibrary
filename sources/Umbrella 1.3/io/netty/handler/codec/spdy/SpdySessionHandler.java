...
extends ChannelDuplexHandler {
    ...
    public SpdySessionHandler(SpdyVersion version, boolean server) {
        ...
        this.sessionFlowControl = version.useSessionFlowControl();
    }

    @Override
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof SpdyDataFrame) {
            DefaultSpdyWindowUpdateFrame spdyWindowUpdateFrame;
            int deltaWindowSize;
            ...
            if (this.sessionFlowControl) {
                ...
                if (newSessionWindowSize <= this.initialReceiveWindowSize / 2) {
                    deltaWindowSize = this.initialReceiveWindowSize - newSessionWindowSize;
                    this.spdySession.updateReceiveWindowSize(0, deltaWindowSize);
                    spdyWindowUpdateFrame = new DefaultSpdyWindowUpdateFrame(0, deltaWindowSize);
            ...
            if (newWindowSize <= this.initialReceiveWindowSize / 2 && !spdyDataFrame.isLast()) {
                ...
                spdyWindowUpdateFrame = new DefaultSpdyWindowUpdateFrame(streamId, deltaWindowSize);
                ctx.writeAndFlush(spdyWindowUpdateFrame);
            }
    ...
    private void updateSendWindowSize(final ChannelHandlerContext ctx, int streamId, int deltaWindowSize) {
        ...
        synchronized (object) {
            ...
            while (newWindowSize > 0 && (pendingWrite = this.spdySession.getPendingWrite(streamId)) != null) {
                ...
                DefaultSpdyDataFrame partialDataFrame = new DefaultSpdyDataFrame(writeStreamId, spdyDataFrame.content().readSlice(newWindowSize).retain());
