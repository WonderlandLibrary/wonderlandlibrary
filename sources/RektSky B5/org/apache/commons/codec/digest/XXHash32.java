...
implements Checksum {
    ...
    public void update(byte[] b2, int off, int len) {
        ...
        if (this.pos > 0) {
            ...
            System.arraycopy(b2, off, this.buffer, this.pos, size);
        ...
        if (off < end) {
            this.pos = end - off;
            System.arraycopy(b2, off, this.buffer, 0, this.pos);
        }
    }
    ...
    private void process(byte[] b2, int offset) {
        int s0 = this.state[0];
        int s1 = this.state[1];
        int s2 = this.state[2];
        int s3 = this.state[3];
        s0 = Integer.rotateLeft(s0 + XXHash32.getInt(b2, offset) * -2048144777, 13) * -1640531535;
        s1 = Integer.rotateLeft(s1 + XXHash32.getInt(b2, offset + 4) * -2048144777, 13) * -1640531535;
        s2 = Integer.rotateLeft(s2 + XXHash32.getInt(b2, offset + 8) * -2048144777, 13) * -1640531535;
        s3 = Integer.rotateLeft(s3 + XXHash32.getInt(b2, offset + 12) * -2048144777, 13) * -1640531535;
        this.state[0] = s0;
        this.state[1] = s1;
        this.state[2] = s2;
        this.state[3] = s3;
