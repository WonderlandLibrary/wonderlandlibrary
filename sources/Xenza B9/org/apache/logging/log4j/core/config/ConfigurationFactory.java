...
public abstract class ConfigurationFactory extends ConfigurationBuilderFactory
{
    ...
    public ConfigurationFactory() {
        this.substitutor = new ConfigurationStrSubstitutor(new Interpolator());
    }
    ...
    public static AuthorizationProvider authorizationProvider(final PropertiesUtil props) {
        final String authClass = props.getStringProperty(ConfigurationFactory.PREFIXES, "authorizationProvider", null);
        ...
        public Configuration getConfiguration(final LoggerContext loggerContext, final String name, final URI configLocation) {
            if (configLocation == null) {
                ...
                for (final ConfigurationFactory factory : ConfigurationFactory.getFactories()) {
                    ...
                    if (types != null) {
                        for (final String type : types) {
                            if (type.equals("*") || configLocationStr2.endsWith(type)) {
                                final Configuration config2 = factory.getConfiguration(loggerContext, name, configLocation);
                                if (config2 != null) {
                                    return config2;
                                }
            ...
            Configuration config4 = this.getConfiguration(loggerContext, true, name);
            if (config4 == null) {
                config4 = this.getConfiguration(loggerContext, true, null);
                if (config4 == null) {
                    config4 = this.getConfiguration(loggerContext, false, name);
                    if (config4 == null) {
                        config4 = this.getConfiguration(loggerContext, false, null);
                    }
                }
            }
            if (config4 != null) {
