...
public class ShadersTex
{
    ...
    public static void uploadTexSubForLoadAtlas(final TextureMap textureMap, final String iconName, final int[][] data, final int width, final int height, final int xoffset, final int yoffset, final boolean linear, final boolean clamp) {
        final MultiTexID multitexid = textureMap.multiTex;
        TextureUtil.uploadTextureMipmap(data, width, height, xoffset, yoffset, linear, clamp);
        final boolean flag = false;
        if (Shaders.configNormalMap) {
            final int[][] aint = readImageAndMipmaps(textureMap, iconName + "_n", width, height, data.length, flag, -8421377);
            GlStateManager.bindTexture(multitexid.norm);
            TextureUtil.uploadTextureMipmap(aint, width, height, xoffset, yoffset, linear, clamp);
        }
        if (Shaders.configSpecularMap) {
            final int[][] aint2 = readImageAndMipmaps(textureMap, iconName + "_s", width, height, data.length, flag, 0);
            GlStateManager.bindTexture(multitexid.spec);
    ...
    public static int[][] readImageAndMipmaps(final TextureMap updatingTextureMap, final String name, final int width, final int height, final int numLevels, final boolean border, final int defColor) {
        final MultiTexID multitexid = updatingTextureMap.multiTex;
    ...
    public static ResourceLocation getNSMapLocation(final ResourceLocation location, final String mapName) {
        ...
        return new ResourceLocation(location.getResourceDomain(), s2 + "_" + mapName + ".png");
    }
