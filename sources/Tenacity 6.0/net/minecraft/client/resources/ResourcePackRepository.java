        ...
        while (iterator.hasNext()) {
            ...
            for (final Entry resourcepackrepository$entry : this.repositoryEntriesAll) {
                if (resourcepackrepository$entry.getResourcePackName().equals(s)) {
                    ...
                    iterator.remove();
                    ResourcePackRepository.logger.warn("Removed selected resource pack {} because it's no longer compatible", new Object[] { resourcepackrepository$entry.getResourcePackName() });
                }
            }
        }
    }
        ...
        try {
            ...
            if (file1.exists() && hash.length() == 40) {
                try {
                    ...
                    ResourcePackRepository.logger.warn("File " + file1 + " couldn't be hashed. Deleting it.", (Throwable)ioexception);
                    FileUtils.deleteQuietly(file1);
                }
            }
            ...
            final ListenableFuture listenablefuture4 = this.downloadingPacks;
            return (ListenableFuture<Object>)listenablefuture4;
        }
        finally {
            this.lock.unlock();
        }
