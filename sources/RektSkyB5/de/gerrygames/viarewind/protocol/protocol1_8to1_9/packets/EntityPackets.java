        ...
        protocol.registerClientbound(ClientboundPackets1_9.VEHICLE_MOVE, ClientboundPackets1_8.ENTITY_TELEPORT, new PacketRemapper(){
            ...
            public void registerMap() {
                ...
                this.handler(packetWrapper -> {
                    ...
                    double x2 = (double)packetWrapper.get(Type.INT, 0).intValue() / 32.0;
                    double y2 = (double)packetWrapper.get(Type.INT, 1).intValue() / 32.0;
                    double z2 = (double)packetWrapper.get(Type.INT, 2).intValue() / 32.0;
                    position.setPos(x2, y2, z2);
                });
                ...
                this.handler(packetWrapper -> {
                    ...
                    if (type == Entity1_10Types.EntityType.BOAT) {
                        ...
                        yaw = (byte)(yaw - 64);
                        packetWrapper.set(Type.BYTE, 0, yaw);
                        int y2 = packetWrapper.get(Type.INT, 1);
                        packetWrapper.set(Type.INT, 1, y2 += 10);
                    }
                });
            }
        });
            ...
            public void registerMap() {
                this.handler(packetWrapper -> {
                    ...
                    for (int i3 = 0; i3 < count; ++i3) {
                        int v2 = i3 == 0 ? vehicle : passengers.get(i3 - 1);
                        int p2 = passengers.get(i3);
                        PacketWrapper attach = PacketWrapper.create(27, null, packetWrapper.user());
                        attach.write(Type.INT, p2);
                        attach.write(Type.INT, v2);
        ...
        protocol.registerClientbound(ClientboundPackets1_9.ENTITY_TELEPORT, (ClientboundPackets1_8)((Object)new PacketRemapper(){
            ...
            public void registerMap() {
                ...
                this.handler(packetWrapper -> {
                    ...
                    if (replacement != null) {
                        packetWrapper.cancel();
                        int x2 = packetWrapper.get(Type.INT, 0);
                        int y2 = packetWrapper.get(Type.INT, 1);
                        int z2 = packetWrapper.get(Type.INT, 2);
                        byte yaw = packetWrapper.get(Type.BYTE, 0);
                        byte pitch = packetWrapper.get(Type.BYTE, 1);
                        replacement.setLocation((double)x2 / 32.0, (double)y2 / 32.0, (double)z2 / 32.0);
                        replacement.setYawPitch((float)yaw * 360.0f / 256.0f, (float)pitch * 360.0f / 256.0f);
                    }
                });
            }
