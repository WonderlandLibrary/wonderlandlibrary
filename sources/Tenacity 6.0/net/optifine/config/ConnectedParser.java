...
public class ConnectedParser
{
    ...
    private IBlockState getStateFromMeta(final Block block, final int md) {
        try {
            ...
            if (block == Blocks.double_plant && md > 7) {
                ...
                iblockstate = iblockstate.withProperty(BlockDoublePlant.VARIANT, (Comparable)iblockstate2.getValue((IProperty<V>)BlockDoublePlant.VARIANT));
            }
            return iblockstate;
        }
    ...
    public static Comparable getPropertyValue(final String value, final Collection propertyValues) {
        for (final Object comparable0 : propertyValues) {
            final Comparable comparable2 = (Comparable)comparable0;
            if (getValueName(comparable2).equals(value)) {
                return comparable2;
            }
        }
        return null;
