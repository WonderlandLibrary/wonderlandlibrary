...
public class GuiWinGame extends GuiScreen
{
    ...
    public void updateScreen() {
        final MusicTicker musicticker = this.mc2.getMusicTicker();
        final SoundHandler soundhandler = this.mc2.getSoundHandler();
    ...
    private void sendRespawnPacket() {
        this.mc2.thePlayer.sendQueue.addToSendQueue(new C16PacketClientStatus(C16PacketClientStatus.EnumState.PERFORM_RESPAWN));
    ...
    public void initGui() {
        if (this.field_146582_i == null) {
            ...
            try {
                ...
                final int i = 274;
                InputStream inputstream = this.mc2.getResourceManager().getResource(new ResourceLocation("texts/end.txt")).getInputStream();
                BufferedReader bufferedreader = new BufferedReader(new InputStreamReader(inputstream, Charsets.UTF_8));
                final Random random = new Random(8124371L);
                while ((s = bufferedreader.readLine()) != null) {
                    String s3;
                    String s4;
                    for (s = s.replaceAll("PLAYERNAME", this.mc2.getSession().getUsername()); s.contains(s2); s = s3 + EnumChatFormatting.WHITE + EnumChatFormatting.OBFUSCATED + "XXXXXXXX".substring(0, random.nextInt(4) + 3) + s4) {
                        final int j = s.indexOf(s2);
                        s3 = s.substring(0, j);
                        s4 = s.substring(j + s2.length());
                    }
                    this.field_146582_i.addAll(this.mc2.fontRendererObj.listFormattedStringToWidth(s, i));
                ...
                inputstream = this.mc2.getResourceManager().getResource(new ResourceLocation("texts/credits.txt")).getInputStream();
                bufferedreader = new BufferedReader(new InputStreamReader(inputstream, Charsets.UTF_8));
                while ((s = bufferedreader.readLine()) != null) {
                    s = s.replaceAll("PLAYERNAME", this.mc2.getSession().getUsername());
                    s = s.replaceAll("\t", "    ");
                    this.field_146582_i.addAll(this.mc2.fontRendererObj.listFormattedStringToWidth(s, i));
                    this.field_146582_i.add("");
                }
    ...
    private void drawWinGameScreen(final int p_146575_1_, final int p_146575_2_, final float p_146575_3_) {
        ...
        this.mc2.getTextureManager().bindTexture(Gui.optionsBackground);
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        this.mc2.getTextureManager().bindTexture(GuiWinGame.MINECRAFT_LOGO);
        ...
        for (int i2 = 0; i2 < this.field_146582_i.size(); ++i2) {
            ...
            if (l + f + 12.0f + 8.0f > 0.0f && l + f < this.height) {
                ...
                if (s.startsWith("[C]")) {
                    this.fontRendererObj.drawStringWithShadow(s.substring(3), j + (i - this.fontRendererObj.getStringWidth(s.substring(3))) / 2.0f, (float)l, 16777215);
                }
                else {
        ...
        this.mc2.getTextureManager().bindTexture(GuiWinGame.VIGNETTE_TEXTURE);
