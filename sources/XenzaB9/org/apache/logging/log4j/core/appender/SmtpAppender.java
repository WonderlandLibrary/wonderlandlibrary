...
public final class SmtpAppender extends AbstractAppender
{
    ...
    private SmtpAppender(final String name, final Filter filter, final Layout<? extends Serializable> layout, final MailManager manager, final boolean ignoreExceptions, final Property[] properties) {
    ...
    public static class Builder extends AbstractAppender.Builder<Builder> implements org.apache.logging.log4j.core.util.Builder<SmtpAppender>
    {
        ...
        public SmtpAppender build() {
            ...
            final AbstractStringLayout.Serializer subjectSerializer = PatternLayout.newSerializerBuilder().setConfiguration(this.getConfiguration()).setPattern(this.subject).build();
            final MailManager.FactoryData data = new MailManager.FactoryData(this.to, this.cc, this.bcc, this.from, this.replyTo, this.subject, subjectSerializer, this.smtpProtocol, this.smtpHost, this.smtpPort, this.smtpUsername, this.smtpPassword, this.smtpDebug, this.bufferSize, this.sslConfiguration, this.getFilter().toString());
            final MailManagerFactory factory = ServiceLoaderUtil.loadServices(MailManagerFactory.class, MethodHandles.lookup()).findAny().orElseGet(() -> SmtpManager.FACTORY);
            final MailManager smtpManager = AbstractManager.getManager(data.getManagerName(), (ManagerFactory<MailManager, MailManager.FactoryData>)factory, data);
            if (smtpManager == null) {
