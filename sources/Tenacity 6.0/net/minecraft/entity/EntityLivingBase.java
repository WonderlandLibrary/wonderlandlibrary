...
public abstract class EntityLivingBase extends Entity
{
    ...
    public boolean attackEntityFrom(final DamageSource source, float amount) {
        ...
        if (flag) {
            ...
            if (entity != null) {
                ...
                this.attackedAtYaw = (float)(MathHelper.atan2(d2, d1) * 180.0 / 3.141592653589793 - this.rotationYaw);
    ...
    private int getArmSwingAnimationEnd() {
        return (Tenacity.INSTANCE.getModuleCollection().get(Animations.class).isEnabled() ? Animations.slowdown.getValue().intValue() : 1) * (this.isPotionActive(Potion.digSpeed) ? (6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier())) : (this.isPotionActive(Potion.digSlowdown) ? (6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2) : 6));
    }
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            final JumpFixEvent jumpFixEvent = new JumpFixEvent(this.rotationYaw);
            if (this instanceof EntityPlayerSP) {
                Tenacity.INSTANCE.getEventProtocol().handleEvent(jumpFixEvent);
            }
            final float f = jumpFixEvent.getYaw() * 0.017453292f;
    ...
    public void onUpdate() {
        ...
        if (f > 0.0025000002f) {
            ...
            f2 = (float)MathHelper.atan2(d2, d0) * 180.0f / 3.1415927f - 90.0f;
        }
    ...
    public boolean isOnTeam(final Team teamIn) {
