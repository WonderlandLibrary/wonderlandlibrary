...
public final class PlatformDependent {
    ...
    private static boolean isRoot0() {
        ...
        for (String idCmd : ID_COMMANDS) {
            ...
            String uid = null;
            try {
                p2 = Runtime.getRuntime().exec(new String[]{idCmd, "-u"});
                in = new BufferedReader(new InputStreamReader(p2.getInputStream(), CharsetUtil.US_ASCII));
                uid = in.readLine();
                in.close();
                while (true) {
                    try {
        ...
        for (int i2 = 1023; i2 > 0; --i2) {
            ...
            try {
                ...
                ss.bind(new InetSocketAddress(i2));
                if (logger.isDebugEnabled()) {
                    logger.debug("UID: 0 (succeded to bind at port {})", (Object)i2);
                }
                boolean idCmd = true;
                return idCmd;
            }
    ...
    private static int bitMode0() {
        ...
        Matcher m2 = BIT_PATTERN.matcher(vm);
        if (m2.find()) {
