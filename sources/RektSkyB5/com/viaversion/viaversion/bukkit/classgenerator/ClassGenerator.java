        ...
        if (ViaVersionPlugin.getInstance().isCompatSpigotBuild() || ViaVersionPlugin.getInstance().isProtocolSupport()) {
            try {
                ...
                for (Plugin p2 : Bukkit.getPluginManager().getPlugins()) {
                    pool.insertClassPath(new LoaderClassPath(p2.getClass().getClassLoader()));
                }
            ...
            catch (CannotCompileException | NotFoundException | ReflectiveOperationException e2) {
                e2.printStackTrace();
            }
        }
    }
