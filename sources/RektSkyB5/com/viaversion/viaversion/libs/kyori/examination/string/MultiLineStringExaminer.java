...
extends AbstractExaminer<Stream<String>> {
    ...
    protected Stream<String> stream(@NotNull DoubleStream stream) {
        return this.arrayLike(stream.mapToObj(d2 -> this.examine(d2)));
    }

    @Override
    @NotNull
    protected Stream<String> stream(@NotNull IntStream stream) {
        return this.arrayLike(stream.mapToObj(n2 -> this.examine(n2)));
    }

    @Override
    @NotNull
    protected Stream<String> stream(@NotNull LongStream stream) {
        return this.arrayLike(stream.mapToObj(l2 -> this.examine(l2)));
    }
    ...
    private static Stream<String> association(List<String> left, String middle, List<String> right) {
        ...
        for (int i2 = 0; i2 < height; ++i2) {
            String l2 = i2 < lefts ? Strings.padEnd(left.get(i2), leftWidth, ' ') : leftPad;
            String m2 = i2 == 0 ? middle : middlePad;
            String r2 = i2 < rights ? right.get(i2) : "";
            result.add(l2 + m2 + r2);
        }
