...
class Sweep {
    ...
    private static ActiveRegion RegionBelow(ActiveRegion r2) {
        return (ActiveRegion)Dict.dictKey(Dict.dictPred(r2.nodeUp));
    }

    private static ActiveRegion RegionAbove(ActiveRegion r2) {
        return (ActiveRegion)Dict.dictKey(Dict.dictSucc(r2.nodeUp));
    }
    ...
    static ActiveRegion TopLeftRegion(ActiveRegion reg) {
        ...
        if (reg.fixUpperEdge) {
            GLUhalfEdge e2 = Mesh.__gl_meshConnect(Sweep.RegionBelow((ActiveRegion)reg).eUp.Sym, reg.eUp.Lnext);
            if (e2 == null) {
                return null;
            }
            if (!Sweep.FixUpperEdge(reg, e2)) {
                return null;
            }
            reg = Sweep.RegionAbove(reg);
        }
    ...
    static boolean IsWindingInside(GLUtessellatorImpl tess, int n2) {
    ...
    static void FinishRegion(GLUtessellatorImpl tess, ActiveRegion reg) {
        GLUhalfEdge e2 = reg.eUp;
        GLUface f2 = e2.Lface;
        f2.inside = reg.inside;
        f2.anEdge = e2;
    ...
    static GLUhalfEdge FinishLeftRegions(GLUtessellatorImpl tess, ActiveRegion regFirst, ActiveRegion regLast) {
        ...
        while (regPrev != regLast) {
            ...
            if (e2.Org != ePrev.Org) {
                ...
                e2 = Mesh.__gl_meshConnect(ePrev.Onext.Sym, e2.Sym);
                if (e2 == null) {
                    throw new RuntimeException();
                }
                if (!Sweep.FixUpperEdge(reg, e2)) {
            ...
            if (ePrev.Onext != e2) {
                ...
                if (!Mesh.__gl_meshSplice(ePrev, e2)) {
                    throw new RuntimeException();
                }
            }
            Sweep.FinishRegion(tess, regPrev);
    ...
    static void AddRightEdges(GLUtessellatorImpl tess, ActiveRegion regUp, GLUhalfEdge eFirst, GLUhalfEdge eLast, GLUhalfEdge eTopLeft, boolean cleanUp) {
        ...
        GLUhalfEdge e2 = eFirst;
        do {
            assert (Geom.VertLeq(e2.Org, e2.Sym.Org));
            Sweep.AddRegionBelow(tess, regUp, e2.Sym);
        } while ((e2 = e2.Onext) != eLast);
        if (eTopLeft == null) {
        ...
        regPrev.dirty = true;
        assert (regPrev.windingNumber - e2.winding == reg.windingNumber);
        if (cleanUp) {
    ...
    static boolean CheckForLeftSplice(GLUtessellatorImpl tess, ActiveRegion regUp) {
        ...
        if (Geom.VertLeq(eUp.Sym.Org, eLo.Sym.Org)) {
            ...
            regLo.dirty = true;
            regUp.dirty = true;
            GLUhalfEdge e3 = Mesh.__gl_meshSplitEdge(eLo);
            if (e3 == null) {
                throw new RuntimeException();
            }
            ...
            e3.Sym.Lface.inside = regUp.inside;
        }
        return true;
    }
    ...
    static boolean CheckForIntersect(GLUtessellatorImpl tess, ActiveRegion regUp) {
        ...
        if (!Geom.VertEq(dstUp, tess.event) && Geom.EdgeSign(dstUp, tess.event, isect) >= 0.0 || !Geom.VertEq(dstLo, tess.event) && Geom.EdgeSign(dstLo, tess.event, isect) <= 0.0) {
            ...
            if (dstUp == tess.event) {
                ...
                GLUhalfEdge e2 = Sweep.RegionBelow((ActiveRegion)regUp).eUp.Sym.Onext;
                regLo.eUp = eLo.Sym.Lnext;
                eLo = Sweep.FinishLeftRegions(tess, regLo, null);
                Sweep.AddRightEdges(tess, regUp, eLo.Onext, eUp.Sym.Onext, e2, true);
                return true;
            }
    ...
    static void ConnectLeftDegenerate(GLUtessellatorImpl tess, ActiveRegion regUp, GLUvertex vEvent) {
        GLUhalfEdge eLast;
        GLUhalfEdge e2 = regUp.eUp;
        if (Geom.VertEq(e2.Org, vEvent)) {
            assert (false);
            Sweep.SpliceMergeVertices(tess, e2, vEvent.anEdge);
            return;
        }
        if (!Geom.VertEq(e2.Sym.Org, vEvent)) {
            if (Mesh.__gl_meshSplitEdge(e2.Sym) == null) {
                throw new RuntimeException();
            }
            if (regUp.fixUpperEdge) {
                if (!Mesh.__gl_meshDelete(e2.Onext)) {
                    throw new RuntimeException();
                }
                regUp.fixUpperEdge = false;
            }
            if (!Mesh.__gl_meshSplice(vEvent.anEdge, e2)) {
                throw new RuntimeException();
            }
    ...
    static void SweepEvent(GLUtessellatorImpl tess, GLUvertex vEvent) {
        tess.event = vEvent;
        Sweep.DebugEvent(tess);
        GLUhalfEdge e2 = vEvent.anEdge;
        while (e2.activeRegion == null) {
            e2 = e2.Onext;
            if (e2 != vEvent.anEdge) continue;
            Sweep.ConnectLeftVertex(tess, vEvent);
            return;
        }
        ActiveRegion regUp = Sweep.TopLeftRegion(e2.activeRegion);
        if (regUp == null) {
            throw new RuntimeException();
        }
        ActiveRegion reg = Sweep.RegionBelow(regUp);
    ...
    static void AddSentinel(GLUtessellatorImpl tess, double t2) {
        ActiveRegion reg = new ActiveRegion();
        if (reg == null) {
            throw new RuntimeException();
        }
        GLUhalfEdge e2 = Mesh.__gl_meshMakeEdge(tess.mesh);
        if (e2 == null) {
            throw new RuntimeException();
        }
        e2.Org.s = 4.0E150;
        e2.Org.t = t2;
        e2.Sym.Org.s = -4.0E150;
        e2.Sym.Org.t = t2;
        tess.event = e2.Sym.Org;
        reg.eUp = e2;
        reg.windingNumber = 0;
        reg.inside = false;
        reg.fixUpperEdge = false;
    ...
    static void RemoveDegenerateEdges(GLUtessellatorImpl tess) {
        ...
        GLUhalfEdge e2 = eHead.next;
        while (e2 != eHead) {
            GLUhalfEdge eNext = e2.next;
            GLUhalfEdge eLnext = e2.Lnext;
            if (Geom.VertEq(e2.Org, e2.Sym.Org) && e2.Lnext.Lnext != e2) {
                Sweep.SpliceMergeVertices(tess, eLnext, e2);
                if (!Mesh.__gl_meshDelete(e2)) {
                    throw new RuntimeException();
                }
                e2 = eLnext;
                eLnext = e2.Lnext;
    ...
    static boolean InitPriorityQ(GLUtessellatorImpl tess) {
        ...
        while (v2 != vHead) {
            v2.pqHandle = pq.pqInsert(v2);
            if ((long)v2.pqHandle == Long.MAX_VALUE) break;
            v2 = v2.next;
        }
        if (v2 != vHead || !pq.pqInit()) {
            tess.pq.pqDeletePriorityQ();
            tess.pq = null;
            return false;
        }
    ...
    static boolean RemoveDegenerateFaces(GLUmesh mesh) {
        ...
        while (f2 != mesh.fHead) {
            GLUface fNext = f2.next;
            GLUhalfEdge e2 = f2.anEdge;
            assert (e2.Lnext != e2);
            if (e2.Lnext.Lnext == e2) {
                Sweep.AddWinding(e2.Onext, e2);
                if (!Mesh.__gl_meshDelete(e2)) {
                    return false;
                }
            }
    ...
    public static boolean __gl_computeInterior(GLUtessellatorImpl tess) {
        ...
        while ((v2 = (GLUvertex)tess.pq.pqExtractMin()) != null) {
            GLUvertex vNext;
            while ((vNext = (GLUvertex)tess.pq.pqMinimum()) != null && Geom.VertEq(vNext, v2)) {
                vNext = (GLUvertex)tess.pq.pqExtractMin();
                Sweep.SpliceMergeVertices(tess, v2.anEdge, vNext.anEdge);
            }
            Sweep.SweepEvent(tess, v2);
        }
