...
implements Closeable {
    ...
    private class FilePart {
        ...
        private String readLine() throws IOException {
            ...
            while (i2 > -1) {
                if (!isLastFilePart && i2 < ReversedLinesFileReader.this.avoidNewlineSplitBufferSize) {
                    this.createLeftOver();
                    break;
                }
                int newLineMatchByteCount = this.getNewLineMatchByteCount(this.data, i2);
                ...
                if ((i2 -= ReversedLinesFileReader.this.byteDecrement) >= 0) continue;
                this.createLeftOver();
                break;
            }
        ...
        private int getNewLineMatchByteCount(byte[] data, int i2) {
            for (byte[] newLineSequence : ReversedLinesFileReader.this.newLineSequences) {
                boolean match = true;
                for (int j2 = newLineSequence.length - 1; j2 >= 0; --j2) {
                    int k2 = i2 + j2 - (newLineSequence.length - 1);
                    match &= k2 >= 0 && data[k2] == newLineSequence[j2];
                }
                if (!match) continue;
