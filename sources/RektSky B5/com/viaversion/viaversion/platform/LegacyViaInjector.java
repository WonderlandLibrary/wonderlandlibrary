...
implements ViaInjector {
    ...
    public void inject() throws ReflectiveOperationException {
        ...
        for (Field field : connection.getClass().getDeclaredFields()) {
            ...
            SynchronizedListWrapper<Object> wrappedList = new SynchronizedListWrapper<Object>(list, o2 -> {
                try {
                    this.injectChannelFuture((ChannelFuture)o2);
                }
    ...
    public void uninject() throws ReflectiveOperationException {
        for (ChannelFuture channelFuture : this.injectedFutures) {
            List<String> names = channelFuture.channel().pipeline().names();
            ChannelHandler bootstrapAcceptor = null;
            for (String name : names) {
        ...
        for (Pair pair : this.injectedLists) {
            try {
                ...
                Object o2 = field.get(pair.value());
                if (!(o2 instanceof SynchronizedListWrapper)) continue;
                List list = originalList = ((SynchronizedListWrapper)o2).originalList();
                synchronized (list) {
