...
public class TypeUtils {
    ...
    public static String toLongString(TypeVariable<?> var2) {
        ...
        block5: {
            ...
            Object d2 = var2.getGenericDeclaration();
            ...
            if (d2 instanceof Type) {
                buf.append(TypeUtils.toString((Type)d2));
            } else {
                buf.append(d2);
            }
        }
    ...
    private static String classToString(Class<?> c2) {
        ...
        if (c2.getEnclosingClass() != null) {
            buf.append(TypeUtils.classToString(c2.getEnclosingClass())).append('.').append(c2.getSimpleName());
        } else {
            buf.append(c2.getName());
        }
        if (c2.getTypeParameters().length > 0) {
            buf.append('<');
            TypeUtils.appendAllTo(buf, ", ", c2.getTypeParameters());
            buf.append('>');
        }
        return buf.toString();
    }
    ...
    private static String parameterizedTypeToString(ParameterizedType p2) {
        ...
        int[] recursiveTypeIndexes = TypeUtils.findRecursiveTypes(p2);
        if (recursiveTypeIndexes.length > 0) {
            TypeUtils.appendRecursiveTypes(buf, recursiveTypeIndexes, p2.getActualTypeArguments());
        } else {
            TypeUtils.appendAllTo(buf.append('<'), ", ", p2.getActualTypeArguments()).append('>');
        }
        return buf.toString();
    }
    ...
    private static void appendRecursiveTypes(StringBuilder buf, int[] recursiveTypeIndexes, Type[] argumentTypes) {
        for (int i2 = 0; i2 < recursiveTypeIndexes.length; ++i2) {
            TypeUtils.appendAllTo(buf.append('<'), ", ", argumentTypes[i2].toString()).append('>');
        }
        Type[] argumentsFiltered = ArrayUtils.removeAll(argumentTypes, recursiveTypeIndexes);
        if (argumentsFiltered.length > 0) {
            TypeUtils.appendAllTo(buf.append('<'), ", ", argumentsFiltered).append('>');
        }
    }

    private static int[] findRecursiveTypes(ParameterizedType p2) {
        Type[] filteredArgumentTypes = Arrays.copyOf(p2.getActualTypeArguments(), p2.getActualTypeArguments().length);
        int[] indexesToRemove = new int[]{};
        for (int i2 = 0; i2 < filteredArgumentTypes.length; ++i2) {
            if (!(filteredArgumentTypes[i2] instanceof TypeVariable) || !TypeUtils.containsVariableTypeSameParametrizedTypeBound((TypeVariable)filteredArgumentTypes[i2], p2)) continue;
            indexesToRemove = ArrayUtils.add(indexesToRemove, i2);
        }
        return indexesToRemove;
    }

    private static boolean containsVariableTypeSameParametrizedTypeBound(TypeVariable<?> typeVariable, ParameterizedType p2) {
        return ArrayUtils.contains(typeVariable.getBounds(), p2);
    }
