...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            final double d5 = y;
            double d6 = z;
            final SafeWalkEvent safeWalkEvent = new SafeWalkEvent();
            Tenacity.INSTANCE.getEventProtocol().handleEvent(safeWalkEvent);
            final boolean flag = this.onGround && (this.isSneaking() || safeWalkEvent.isSafe()) && this instanceof EntityPlayer;
            if (flag) {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        final PlayerMoveUpdateEvent playerMovementEvent = new PlayerMoveUpdateEvent(strafe, forward, friction, this.rotationYaw, this.rotationPitch);
        ...
        if (f >= 1.0E-4f) {
            ...
            strafe *= f;
            forward *= f;
            final float f2 = MathHelper.sin(playerMovementEvent.getYaw() * 3.1415927f / 180.0f);
            final float f3 = MathHelper.cos(playerMovementEvent.getYaw() * 3.1415927f / 180.0f);
    ...
    public MovingObjectPosition rayTraceScaffoldRotations(final double blockReachDistance, final float partialTicks, final float yaw, final float pitch) {
        final Vec3 vec3 = this.getPositionEyes(partialTicks);
        final Vec3 vec4 = this.getVectorForRotation(pitch, yaw);
        final Vec3 vec5 = vec3.addVector(vec4.xCoord * blockReachDistance, vec4.yCoord * blockReachDistance, vec4.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec5, false, false, true);
    }
    ...
    public void setPortal(final BlockPos pos) {
        if (this.timeUntilPortal > 0) {
            ...
            if (!this.worldObj.isRemote && !pos.equals(this.lastPortalPos)) {
                ...
                final BlockPattern.PatternHelper blockpattern$patternhelper = Blocks.portal.func_181089_f(this.worldObj, pos);
                final double d0 = (blockpattern$patternhelper.getFinger().getAxis() == EnumFacing.Axis.X) ? blockpattern$patternhelper.getPos().getZ() : ((double)blockpattern$patternhelper.getPos().getX());
                ...
                final double d3 = MathHelper.func_181160_c(this.posY - 1.0, blockpattern$patternhelper.getPos().getY(), blockpattern$patternhelper.getPos().getY() - blockpattern$patternhelper.func_181119_e());
                this.lastPortalVec = new Vec3(d2, d3, 0.0);
    ...
    protected boolean pushOutOfBlocks(final double x, final double y, final double z) {
        ...
        final List<AxisAlignedBB> list = this.worldObj.getCollisionBoxes(this.getEntityBoundingBox());
