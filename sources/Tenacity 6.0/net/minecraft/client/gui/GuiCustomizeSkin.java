...
public class GuiCustomizeSkin extends GuiScreen
{
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        if (button.enabled) {
            if (button.id == 210) {
                this.mc2.displayGuiScreen(new GuiScreenCapeOF(this));
            }
            if (button.id == 200) {
                ...
                this.mc2.gameSettings.switchModelPartEnabled(enumplayermodelparts);
    ...
    private String func_175358_a(final EnumPlayerModelParts playerModelParts) {
        String s;
        if (this.mc2.gameSettings.getModelParts().contains(playerModelParts)) {
