...
public class IFontRenderer implements IResourceManagerReloadListener, AbstractFontRenderer
{
    ...
    public IFontRenderer(final GameSettings gameSettingsIn, final ResourceLocation location, final TextureManager textureManagerIn, final boolean unicode) {
    ...
    public void onResourceManagerReload(final IResourceManager resourceManager) {
        ...
        Arrays.fill(IFontRenderer.unicodePageLocations, null);
    ...
    private ResourceLocation getUnicodePageLocation(final int page) {
        if (IFontRenderer.unicodePageLocations[page] == null) {
            IFontRenderer.unicodePageLocations[page] = new ResourceLocation(String.format("textures/font/unicode_page_%02x.png", page));
            IFontRenderer.unicodePageLocations[page] = FontUtils.getHdFontLocation(IFontRenderer.unicodePageLocations[page]);
        }
        return IFontRenderer.unicodePageLocations[page];
    }
    ...
    private int renderStringAligned(final String text, int x, final int y, final int width, final int color, final boolean dropShadow) {
        if (this.bidiFlag) {
            final int i = (int)this.getStringWidth(this.bidiReorder(text));
            x = x + width - i;
        }
    ...
    @Override
    public int drawCenteredString(final String text, final float x2, final float y2, final int color) {
        return this.drawString(text, (float)(int)(x2 - this.getStringWidth(text) / 2.0f), y2, color);
    }
    
    @Override
    public void drawCenteredString(final String text, final float x2, final float y2, final Color color) {
        this.drawString(text, (float)(int)(x2 - this.getStringWidth(text) / 2.0f), y2, color);
    }
    ...
    public void drawOutlinedString(final String text, final float x, final float y, final int color, final boolean shadow) {
        final int shadowColor = (color & 0xFCFCFC) >> 2 | (color & 0xFF000000);
        this.drawString(text, x + 1.0f, y, shadowColor, false);
        this.drawString(text, x - 1.0f, y, shadowColor, false);
        this.drawString(text, x, y + 1.0f, shadowColor, false);
        this.drawString(text, x, y - 1.0f, shadowColor, false);
