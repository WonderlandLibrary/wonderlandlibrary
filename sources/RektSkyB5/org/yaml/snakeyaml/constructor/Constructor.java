...
extends SafeConstructor {
    ...
    private static final String check(String s2) {
        if (s2 == null) {
            throw new NullPointerException("Root type must be provided.");
        }
        if (s2.trim().length() == 0) {
            ...
            if (!possibleConstructors.isEmpty()) {
                ...
                for (java.lang.reflect.Constructor constructor : possibleConstructors) {
                    ...
                    for (int i2 = 0; i2 < argTypes.length; ++i2) {
                        if (this.wrapIfPrimitive(argTypes[i2]).isAssignableFrom(parameterTypes[i2])) continue;
        ...
        public Object construct(Node nnode) {
            ...
            try {
                ...
                if (type.isPrimitive() || type == String.class || Number.class.isAssignableFrom(type) || type == Boolean.class || Date.class.isAssignableFrom(type) || type == Character.class || type == BigInteger.class || type == BigDecimal.class || Enum.class.isAssignableFrom(type) || Tag.BINARY.equals(node.getTag()) || Calendar.class.isAssignableFrom(type) || type == UUID.class) {
                    ...
                    for (java.lang.reflect.Constructor<?> c2 : javaConstructors) {
                        if (c2.getParameterTypes().length != 1) continue;
                        ++oneArgCount;
                        javaConstructor = c2;
                    }
    ...
    implements Construct {
        ...
        public Object construct(Node node) {
            try {
                ...
                throw new ConstructorException(null, null, "Can't construct a java object for " + node.getTag() + "; exception=" + e3.getMessage(), node.getStartMark(), e3);
            }
        }
        ...
        public void construct2ndStep(Node node, Object object) {
            try {
                ...
                throw new ConstructorException(null, null, "Can't construct a second step for a java object for " + node.getTag() + "; exception=" + e2.getMessage(), node.getStartMark(), e2);
            }
        }
    }
        ...
        protected Object constructJavaBean2ndStep(MappingNode node, Object object) {
            ...
            for (NodeTuple tuple : nodeValue) {
                ...
                try {
                    ...
                    throw new ConstructorException("Cannot create property=" + key + " for JavaBean=" + object, node.getStartMark(), e3.getMessage(), valueNode.getStartMark(), e3);
                }
            }
            return object;
        }
