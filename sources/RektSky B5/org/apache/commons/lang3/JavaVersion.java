...
public enum JavaVersion {
    ...
    static JavaVersion get(String nom) {
        ...
        float v2 = JavaVersion.toFloatVersion(nom);
        if ((double)v2 - 1.0 < 1.0 ? Float.parseFloat(nom.substring((firstComma = Math.max(nom.indexOf(46), nom.indexOf(44))) + 1, end = Math.max(nom.length(), nom.indexOf(44, firstComma)))) > 0.9f : v2 > 10.0f) {
            return JAVA_RECENT;
        }
        return null;
    }
    ...
    private static float maxVersion() {
        float v2 = JavaVersion.toFloatVersion(System.getProperty("java.specification.version", "99.0"));
        if (v2 > 0.0f) {
            return v2;
        }
        return 99.0f;
