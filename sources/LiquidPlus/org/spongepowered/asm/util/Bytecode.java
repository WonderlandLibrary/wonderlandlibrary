...
public final class Bytecode {
    ...
    public static String getDescriptor(Type[] args2, Type returnType) {
    ...
    public static String getSimpleName(AnnotationNode annotation2) {
        return Bytecode.getSimpleName(annotation2.desc);
    }
    ...
    private static List<AnnotationNode> mergeAnnotations(List<AnnotationNode> from, List<AnnotationNode> to, String type, String name) {
        try {
            ...
            for (AnnotationNode annotation2 : from) {
                if (!Bytecode.isMergeableAnnotation(annotation2)) continue;
                Iterator<AnnotationNode> iter = to.iterator();
                while (iter.hasNext()) {
                    if (!iter.next().desc.equals(annotation2.desc)) continue;
                    iter.remove();
                    break;
                }
                to.add(annotation2);
    ...
    private static boolean isMergeableAnnotation(AnnotationNode annotation2) {
        if (annotation2.desc.startsWith("L" + Constants.MIXIN_PACKAGE_REF)) {
            return mergeableAnnotationPattern.matcher(annotation2.desc).matches();
        }
        return true;
    }
