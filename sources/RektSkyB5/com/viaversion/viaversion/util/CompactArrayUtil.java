...
public class CompactArrayUtil {
    ...
    public static long[] createCompactArrayWithPadding(int bitsPerEntry, int entries, IntToLongFunction valueGetter) {
        ...
        for (int i2 = 0; i2 < entries; ++i2) {
            long value = valueGetter.applyAsLong(i2);
            int cellIndex = (int)((long)i2 * divideMul + divideAdd >> 32 >> divideShift);
            int bitIndex = (i2 - cellIndex * valuesPerLong) * bitsPerEntry;
    ...
    public static long[] createCompactArray(int bitsPerEntry, int entries, IntToLongFunction valueGetter) {
        ...
        for (int i2 = 0; i2 < entries; ++i2) {
            long value = valueGetter.applyAsLong(i2);
            int bitIndex = i2 * bitsPerEntry;
            int startIndex = bitIndex / 64;
            int endIndex = ((i2 + 1) * bitsPerEntry - 1) / 64;
