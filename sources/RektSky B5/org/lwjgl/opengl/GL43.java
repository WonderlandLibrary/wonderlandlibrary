...
public final class GL43 {
    ...
    public static void glInvalidateSubFramebuffer(int target, IntBuffer attachments, int x2, int y2, int width, int height) {
        ContextCapabilities caps = GLContext.getCapabilities();
        long function_pointer = caps.glInvalidateSubFramebuffer;
        BufferChecks.checkFunctionAddress(function_pointer);
        BufferChecks.checkDirect(attachments);
        GL43.nglInvalidateSubFramebuffer(target, attachments.remaining(), MemoryUtil.getAddress(attachments), x2, y2, width, height, function_pointer);
    }
