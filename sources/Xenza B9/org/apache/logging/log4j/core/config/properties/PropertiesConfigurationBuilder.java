...
public class PropertiesConfigurationBuilder extends ConfigurationBuilderFactory implements Builder<PropertiesConfiguration>
{
    ...
    public PropertiesConfiguration build() {
        ...
        for (final Map.Entry<String, Properties> entry : scripts.entrySet()) {
            ...
            final String type = ((Hashtable<K, String>)scriptProps).remove("type");
            if (type == null) {
        ...
        if (levelProps.size() > 0) {
            for (final String key3 : levelProps.stringPropertyNames()) {
                this.builder.add(this.builder.newCustomLevel(key3, Integers.parseInt(levelProps.getProperty(key3))));
            }
        }
        ...
        if (loggerProp != null) {
            ...
            final Map<String, Properties> loggers = PropertiesUtil.partitionOnCommonPrefixes(PropertiesUtil.extractSubset(this.rootProperties, "logger"), true);
        ...
        final String rootProp = this.rootProperties.getProperty("rootLogger");
    ...
    private ScriptComponentBuilder createScript(final Properties properties) {
        final String name = ((Hashtable<K, String>)properties).remove("name");
        final String language = ((Hashtable<K, String>)properties).remove("language");
        final String text = ((Hashtable<K, String>)properties).remove("text");
    ...
    private ScriptFileComponentBuilder createScriptFile(final Properties properties) {
        final String name = ((Hashtable<K, String>)properties).remove("name");
        final String path = ((Hashtable<K, String>)properties).remove("path");
    ...
    private AppenderComponentBuilder createAppender(final String key, final Properties properties) {
        ...
        final String type = ((Hashtable<K, String>)properties).remove("type");
    ...
    private FilterComponentBuilder createFilter(final String key, final Properties properties) {
        ...
        final String onMatch = ((Hashtable<K, String>)properties).remove("onMatch");
        final String onMismatch = ((Hashtable<K, String>)properties).remove("onMismatch");
    ...
    private AppenderRefComponentBuilder createAppenderRef(final String key, final Properties properties) {
        final String ref = ((Hashtable<K, String>)properties).remove("ref");
        ...
        final String level = Strings.trimToNull(((Hashtable<K, String>)properties).remove("level"));
    ...
    private LoggerComponentBuilder createLogger(final String key, final Properties properties) {
        ...
        final String name = ((Hashtable<K, String>)properties).remove("name");
        final String location = ((Hashtable<K, String>)properties).remove("includeLocation");
        if (Strings.isEmpty(name)) {
        ...
        final String additivity = ((Hashtable<K, String>)properties).remove("additivity");
