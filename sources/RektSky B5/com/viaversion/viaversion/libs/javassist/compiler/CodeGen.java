...
TokenId {
    ...
    private void atSyncStmnt(Stmnt st) throws CompileError {
        ...
        final int var2 = bc.getMaxLocals();
        bc.incMaxLocals(1);
        bc.addOpcode(89);
        bc.addAstore(var2);
        bc.addOpcode(194);
    ...
    private void atVariableAssign(Expr expr, int op, Variable var2, Declarator d2, ASTree right, boolean doDup) throws CompileError {
        int varType = d2.getType();
        int varArray = d2.getArrayDim();
        String varClass = d2.getClassName();
        int varNo = this.getLocalVar(d2);
        if (op != 61) {
            this.atVariable(var2);
        }
    ...
    private void compareExpr(boolean branchIf, int token, int type1, BinExpr expr) throws CompileError {
        ...
        if ((p2 = CodeGen.typePrecedence(this.exprType)) == -1 || this.arrayDim > 0) {
            ...
            int[] op = ifOp;
            for (int i2 = 0; i2 < op.length; i2 += 3) {
                if (op[i2] != token) continue;
                this.bytecode.addOpcode(op[i2 + (branchIf ? 1 : 2)]);
                return;
            }
            CodeGen.badTypes(expr);
        } else {
            ...
            int[] op = ifOp2;
            for (int i3 = 0; i3 < op.length; i3 += 3) {
                if (op[i3] != token) continue;
                this.bytecode.addOpcode(op[i3 + (branchIf ? 1 : 2)]);
                return;
            }
            CodeGen.badTypes(expr);
        }
    ...
    private void atPlusPlus(int token, ASTree oprand, Expr expr, boolean doDup) throws CompileError {
        ...
        if (oprand instanceof Variable) {
            ...
            int var2 = this.getLocalVar(d2);
            if (this.arrayDim > 0) {
                CodeGen.badType(expr);
            }
            if (t2 == 312) {
                this.bytecode.addDload(var2);
                if (doDup && isPost) {
                    this.bytecode.addOpcode(92);
                }
                ...
                this.bytecode.addDstore(var2);
            } else if (t2 == 326) {
                this.bytecode.addLload(var2);
                if (doDup && isPost) {
                    this.bytecode.addOpcode(92);
                }
                this.bytecode.addLconst(1L);
                ...
                this.bytecode.addLstore(var2);
            } else if (t2 == 317) {
                this.bytecode.addFload(var2);
                if (doDup && isPost) {
                    this.bytecode.addOpcode(89);
                }
