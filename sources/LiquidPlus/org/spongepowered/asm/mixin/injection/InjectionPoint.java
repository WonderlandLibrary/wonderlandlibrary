...
public abstract class InjectionPoint {
    ...
    public static InjectionPoint parse(IMixinContext context, MethodNode method, AnnotationNode parent, AnnotationNode node) {
        ...
        List args2 = (List)Annotations.getValue(node, "args");
        ...
        if (args2 == null) {
            args2 = ImmutableList.of();
        }
        return InjectionPoint.parse(context, method, parent, at, shift, by, args2, target, slice, ordinal, opcode, id);
    }

    public static InjectionPoint parse(IMixinContext context, MethodNode method, AnnotationNode parent, String at, At.Shift shift, int by, List<String> args2, String target, String slice, int ordinal, int opcode, String id) {
        InjectionPointData data = new InjectionPointData(context, method, parent, at, args2, target, slice, ordinal, opcode, id);
    ...
    extends CompositeInjectionPoint {
        ...
        public boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes) {
            ...
            ArrayList alpha2 = allNodes[0];
            for (int nodeIndex = 0; nodeIndex < alpha2.size(); ++nodeIndex) {
                AbstractInsnNode node = (AbstractInsnNode)alpha2.get(nodeIndex);
                boolean in = true;
