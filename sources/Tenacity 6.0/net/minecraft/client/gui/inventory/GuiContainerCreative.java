...
public class GuiContainerCreative extends InventoryEffectRenderer
{
    ...
    public void updateScreen() {
        if (!this.mc2.playerController.isInCreativeMode()) {
            this.mc2.displayGuiScreen(new GuiInventory(this.mc2.thePlayer));
        }
    ...
    protected void handleMouseClick(final Slot slotIn, final int slotId, final int clickedButton, int clickType) {
        ...
        if (slotIn == null && GuiContainerCreative.selectedTabIndex != CreativeTabs.tabInventory.getTabIndex() && clickType != 5) {
            final InventoryPlayer inventoryplayer1 = this.mc2.thePlayer.inventory;
            if (inventoryplayer1.getItemStack() != null) {
                if (clickedButton == 0) {
                    this.mc2.thePlayer.dropPlayerItemWithRandomChoice(inventoryplayer1.getItemStack(), true);
                    this.mc2.playerController.sendPacketDropItem(inventoryplayer1.getItemStack());
                    inventoryplayer1.setItemStack(null);
                }
                if (clickedButton == 1) {
                    final ItemStack itemstack5 = inventoryplayer1.getItemStack().splitStack(1);
                    this.mc2.thePlayer.dropPlayerItemWithRandomChoice(itemstack5, true);
                    this.mc2.playerController.sendPacketDropItem(itemstack5);
            ...
            for (int j = 0; j < this.mc2.thePlayer.inventoryContainer.getInventory().size(); ++j) {
                this.mc2.playerController.sendSlotPacket(null, j);
            }
        }
        else if (GuiContainerCreative.selectedTabIndex == CreativeTabs.tabInventory.getTabIndex()) {
            if (slotIn == this.field_147064_C) {
                this.mc2.thePlayer.inventory.setItemStack(null);
            }
                ...
                this.mc2.thePlayer.dropPlayerItemWithRandomChoice(itemstack6, true);
                this.mc2.playerController.sendPacketDropItem(itemstack6);
            }
            else if (clickType == 4 && this.mc2.thePlayer.inventory.getItemStack() != null) {
                this.mc2.thePlayer.dropPlayerItemWithRandomChoice(this.mc2.thePlayer.inventory.getItemStack(), true);
                this.mc2.playerController.sendPacketDropItem(this.mc2.thePlayer.inventory.getItemStack());
                this.mc2.thePlayer.inventory.setItemStack(null);
            }
            else {
                this.mc2.thePlayer.inventoryContainer.slotClick((slotIn == null) ? slotId : ((CreativeSlot)slotIn).slot.slotNumber, clickedButton, clickType, this.mc2.thePlayer);
                this.mc2.thePlayer.inventoryContainer.detectAndSendChanges();
            }
        }
        else if (clickType != 5 && slotIn.inventory == GuiContainerCreative.field_147060_v) {
            final InventoryPlayer inventoryplayer2 = this.mc2.thePlayer.inventory;
            ...
            if (clickType == 2) {
                if (itemstack8 != null && clickedButton >= 0 && clickedButton < 9) {
                    ...
                    this.mc2.thePlayer.inventory.setInventorySlotContents(clickedButton, itemstack9);
                    this.mc2.thePlayer.inventoryContainer.detectAndSendChanges();
                }
                return;
            }
            ...
            if (clickType == 4) {
                if (itemstack8 != null) {
                    ...
                    this.mc2.thePlayer.dropPlayerItemWithRandomChoice(itemstack11, true);
                    this.mc2.playerController.sendPacketDropItem(itemstack11);
                }
                return;
            }
            if (itemstack7 != null && itemstack7.isItemEqual(itemstack8)) {
                if (clickedButton == 0) {
                    if (flag) {
            ...
            this.inventorySlots.slotClick((slotIn == null) ? slotId : slotIn.slotNumber, clickedButton, clickType, this.mc2.thePlayer);
            if (Container.getDragEvent(clickedButton) == 2) {
                for (int i = 0; i < 9; ++i) {
                    this.mc2.playerController.sendSlotPacket(this.inventorySlots.getSlot(45 + i).getStack(), 36 + i);
                }
            }
            else if (slotIn != null) {
                final ItemStack itemstack12 = this.inventorySlots.getSlot(slotIn.slotNumber).getStack();
                this.mc2.playerController.sendSlotPacket(itemstack12, slotIn.slotNumber - this.inventorySlots.inventorySlots.size() + 9 + 36);
            }
        }
    }
    ...
    public void initGui() {
        if (this.mc2.playerController.isInCreativeMode()) {
            ...
            this.field_147059_E = new CreativeCrafting(this.mc2);
            this.mc2.thePlayer.inventoryContainer.onCraftGuiOpened(this.field_147059_E);
        }
        else {
            this.mc2.displayGuiScreen(new GuiInventory(this.mc2.thePlayer));
        }
    ...
    public void onGuiClosed() {
        super.onGuiClosed();
        if (this.mc2.thePlayer != null && this.mc2.thePlayer.inventory != null) {
            this.mc2.thePlayer.inventoryContainer.removeCraftingFromCrafters(this.field_147059_E);
        }
    ...
    protected void keyTyped(final char typedChar, final int keyCode) throws IOException {
        if (GuiContainerCreative.selectedTabIndex != CreativeTabs.tabAllSearch.getTabIndex()) {
            if (GameSettings.isKeyDown(this.mc2.gameSettings.keyBindChat)) {
    ...
    private void updateCreativeSearch() {
        ...
        while (iterator.hasNext()) {
            ...
            boolean flag = false;
            for (final String s2 : itemstack.getTooltip(this.mc2.thePlayer, this.mc2.gameSettings.advancedItemTooltips)) {
    ...
    protected void drawGuiContainerForegroundLayer(final int mouseX, final int mouseY) {
        ...
        if (creativetabs.drawInForegroundOfTab()) {
            this.fontRendererObj.drawString(I18n.format(creativetabs.getTranslatedTabLabel(), new Object[0]), 8.0f, 6.0f, 4210752);
        }
    ...
    private void setCurrentCreativeTab(final CreativeTabs p_147050_1_) {
        ...
        if (p_147050_1_ == CreativeTabs.tabInventory) {
            final Container container = this.mc2.thePlayer.inventoryContainer;
    ...
    protected void renderToolTip(final ItemStack stack, final int x, final int y) {
        if (GuiContainerCreative.selectedTabIndex == CreativeTabs.tabAllSearch.getTabIndex()) {
            final List<String> list = stack.getTooltip(this.mc2.thePlayer, this.mc2.gameSettings.advancedItemTooltips);
    ...
    protected void drawGuiContainerBackgroundLayer(final float partialTicks, final int mouseX, final int mouseY) {
        ...
        final CreativeTabs creativetabs = CreativeTabs.creativeTabArray[GuiContainerCreative.selectedTabIndex];
        for (final CreativeTabs creativetabs2 : CreativeTabs.creativeTabArray) {
            this.mc2.getTextureManager().bindTexture(GuiContainerCreative.creativeInventoryTabs);
            if (creativetabs2.getTabIndex() != GuiContainerCreative.selectedTabIndex) {
                this.func_147051_a(creativetabs2);
            }
        }
        this.mc2.getTextureManager().bindTexture(new ResourceLocation("textures/gui/container/creative_inventory/tab_" + creativetabs.getBackgroundImageName()));
        ...
        if (creativetabs == CreativeTabs.tabInventory) {
            GuiInventory.drawEntityOnScreen(this.guiLeft + 43, this.guiTop + 45, 20, (float)(this.guiLeft + 43 - mouseX), (float)(this.guiTop + 45 - 30 - mouseY), this.mc2.thePlayer);
        }
    }
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        if (button.id == 0) {
            this.mc2.displayGuiScreen(new GuiAchievements(this, this.mc2.thePlayer.getStatFileWriter()));
        }
        if (button.id == 1) {
            this.mc2.displayGuiScreen(new GuiStats(this, this.mc2.thePlayer.getStatFileWriter()));
        }
    }
