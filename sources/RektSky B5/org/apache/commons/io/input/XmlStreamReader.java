...
extends Reader {
    ...
    static String getContentTypeMime(String httpContentType) {
        ...
        if (httpContentType != null) {
            int i2 = httpContentType.indexOf(";");
            mime = i2 >= 0 ? httpContentType.substring(0, i2) : httpContentType;
            mime = mime.trim();
        }
        return mime;
    }
    ...
    static String getContentTypeEncoding(String httpContentType) {
        ...
        if (httpContentType != null && (i2 = httpContentType.indexOf(";")) > -1) {
            String postMime = httpContentType.substring(i2 + 1);
            Matcher m2 = CHARSET_PATTERN.matcher(postMime);
            encoding = m2.find() ? m2.group(1) : null;
    ...
    private static String getXmlProlog(InputStream is, String guessedEnc) throws IOException {
        ...
        if (guessedEnc != null) {
            ...
            int offset = 0;
            int max = 4096;
            int c2 = is.read(bytes, offset, max);
            int firstGT = -1;
            String xmlProlog = "";
            while (c2 != -1 && firstGT == -1 && offset < 4096) {
                c2 = is.read(bytes, offset += c2, max -= c2);
            ...
            if (bytesRead > 0) {
                ...
                Matcher m2 = ENCODING_PATTERN.matcher(prolog);
                if (m2.find()) {
                    encoding = m2.group(1).toUpperCase();
