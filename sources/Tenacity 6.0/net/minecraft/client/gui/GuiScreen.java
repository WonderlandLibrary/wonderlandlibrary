...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback, Utils
{
    ...
    protected IFontRenderer fontRendererObj;
    ...
    protected void drawHoveringText(final List<String> textLines, final int x, final int y) {
        if (!textLines.isEmpty()) {
            ...
            for (final String s : textLines) {
                final int j = (int)this.fontRendererObj.getStringWidth(s);
                if (j > i) {
                    i = j;
                }
            }
            ...
            Gui.drawGradientRect(l1 - 3, i2 - 4, l1 + i + 3, i2 - 3, m, m);
            Gui.drawGradientRect(l1 - 3, i2 + k + 3, l1 + i + 3, i2 + k + 4, m, m);
            Gui.drawGradientRect(l1 - 3, i2 - 3, l1 + i + 3, i2 + k + 3, m, m);
            Gui.drawGradientRect(l1 - 4, i2 - 3, l1 - 3, i2 + k + 3, m, m);
            Gui.drawGradientRect(l1 + i + 3, i2 - 3, l1 + i + 4, i2 + k + 3, m, m);
            final int i3 = 1347420415;
            final int j2 = (i3 & 0xFEFEFE) >> 1 | (i3 & 0xFF000000);
            Gui.drawGradientRect(l1 - 3, i2 - 3 + 1, l1 - 3 + 1, i2 + k + 3 - 1, i3, j2);
            Gui.drawGradientRect(l1 + i + 2, i2 - 3 + 1, l1 + i + 3, i2 + k + 3 - 1, i3, j2);
            Gui.drawGradientRect(l1 - 3, i2 - 3, l1 + i + 3, i2 - 3 + 1, i3, i3);
            Gui.drawGradientRect(l1 - 3, i2 + k + 2, l1 + i + 3, i2 + k + 3, j2, j2);
    ...
    protected void handleComponentHover(final IChatComponent component, final int x, final int y) {
        if (component != null && component.getChatStyle().getChatHoverEvent() != null) {
            ...
            if (hoverevent.getAction() == HoverEvent.Action.SHOW_ITEM) {
                ...
                if (this.mc2.gameSettings.advancedItemTooltips) {
                    try {
                ...
                this.drawHoveringText(GuiScreen.NEWLINE_SPLITTER.splitToList((CharSequence)hoverevent.getValue().getFormattedText()), x, y);
            }
    ...
    protected boolean handleComponentClick(final IChatComponent component) {
        ...
        if (isShiftKeyDown()) {
            ...
            if (clickevent.getAction() == ClickEvent.Action.OPEN_URL) {
                if (!this.mc2.gameSettings.chatLinks) {
                    return false;
                }
                try {
                    ...
                    if (this.mc2.gameSettings.chatLinksPrompt) {
                        this.clickedLinkURI = uri;
                        this.mc2.displayGuiScreen(new GuiConfirmOpenLink(this, clickevent.getValue(), 31102009, false));
                    }
                    else {
    ...
    public void sendChatMessage(final String msg, final boolean addToChat) {
        if (addToChat) {
            this.mc2.ingameGUI.getChatGUI().addToSentMessages(msg);
        }
        this.mc2.thePlayer.sendChatMessage(msg);
    }
        ...
        if (mouseButton == 0) {
            for (int i = 0; i < this.buttonList.size(); ++i) {
                ...
                if (guibutton.mousePressed(this.mc2, mouseX, mouseY)) {
                    (this.selectedButton = guibutton).playPressSound(this.mc2.getSoundHandler());
    ...
    public void a(final int p_a_1_, final int p_a_2_) {
        this.width = p_a_1_;
        this.height = p_a_2_;
    }
    ...
    public void handleMouseInput() throws IOException {
        final int i = Mouse.getEventX() * this.width / this.mc2.displayWidth;
        final int j = this.height - Mouse.getEventY() * this.height / this.mc2.displayHeight - 1;
        final int k = Mouse.getEventButton();
        if (Mouse.getEventButtonState()) {
            if (this.mc2.gameSettings.touchscreen && this.touchValue++ > 0) {
                return;
            }
            this.eventButton = k;
    ...
    public void drawWorldBackground(final int tint) {
        if (this.mc2.theWorld != null) {
            Gui.drawGradientRect(0.0, 0.0, this.width, this.height, -1072689136, -804253680);
        }
        else {
    ...
    public void drawBackground(final int tint) {
        ...
        this.mc2.getTextureManager().bindTexture(GuiScreen.optionsBackground);
