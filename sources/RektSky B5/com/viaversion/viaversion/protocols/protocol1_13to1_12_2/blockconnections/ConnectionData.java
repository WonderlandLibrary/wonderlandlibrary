...
public class ConnectionData {
    ...
    public static void updateBlockStorage(UserConnection userConnection, int x2, int y2, int z2, int blockState) {
        if (!ConnectionData.needStoreBlocks()) {
            return;
        }
        if (ConnectionData.isWelcome(blockState)) {
            blockConnectionProvider.storeBlock(userConnection, x2, y2, z2, blockState);
        } else {
            blockConnectionProvider.removeBlock(userConnection, x2, y2, z2);
        }
    }
    ...
    public static void connectBlocks(UserConnection user, Chunk chunk) {
        ...
        for (int i2 = 0; i2 < chunk.getSections().length; ++i2) {
            ...
            for (int p2 = 0; p2 < section.getPaletteSize(); ++p2) {
                int id = section.getPaletteEntry(p2);
            ...
            for (int y2 = 0; y2 < 16; ++y2) {
                for (int z2 = 0; z2 < 16; ++z2) {
                    for (int x2 = 0; x2 < 16; ++x2) {
                        ...
                        if (handler == null) continue;
                        block = handler.connect(user, new Position((int)(xOff + (long)x2), (short)(yOff + (long)y2), (int)(zOff + (long)z2)), block);
                        section.setFlatBlock(x2, y2, z2, block);
                    }
                }
            }
        }
    ...
    public static void init() {
        ...
        for (Map.Entry<String, JsonElement> entry : blocks1_13.entrySet()) {
            int n2 = Integer.parseInt(entry.getKey());
        ...
        if (!Via.getConfig().isReduceBlockStorageMemory()) {
            blockConnectionData = new Int2ObjectOpenHashMap<BlockData>(1146, 0.99f);
