...
public class Quat4f extends Tuple4f implements Serializable
{
    ...
    public final void set(final Matrix4d matrix4d) {
        ...
        if (n >= 1.0E-30) {
            ...
            this.x = (float)((matrix4d.m21 - matrix4d.m12) * n2);
            this.y = (float)((matrix4d.m02 - matrix4d.m20) * n2);
            this.z = (float)((matrix4d.m10 - matrix4d.m01) * n2);
            return;
        }
        this.w = 0.0f;
        ...
        if (n3 >= 1.0E-30) {
            ...
            this.y = (float)(matrix4d.m10 * n4);
            this.z = (float)(matrix4d.m20 * n4);
            return;
        }
        this.x = 0.0f;
    ...
    public final void set(final Matrix3d matrix3d) {
        ...
        if (n >= 1.0E-30) {
            this.w = (float)Math.sqrt(n);
            final double n2 = 0.25 / this.w;
            this.x = (float)((matrix3d.m21 - matrix3d.m12) * n2);
            this.y = (float)((matrix3d.m02 - matrix3d.m20) * n2);
            this.z = (float)((matrix3d.m10 - matrix3d.m01) * n2);
            return;
        }
        this.w = 0.0f;
        ...
        if (n3 >= 1.0E-30) {
            this.x = (float)Math.sqrt(n3);
            final double n4 = 0.5 / this.x;
            this.y = (float)(matrix3d.m10 * n4);
            this.z = (float)(matrix3d.m20 * n4);
            return;
        }
        this.x = 0.0f;
    ...
    public final void interpolate(final Quat4f quat4f, final float n) {
        double n2 = this.x * quat4f.x + this.y * quat4f.y + this.z * quat4f.z + this.w * quat4f.w;
        if (n2 < 0.0) {
            quat4f.x = -quat4f.x;
            quat4f.y = -quat4f.y;
            quat4f.z = -quat4f.z;
        ...
        this.w = (float)(n3 * this.w + n4 * quat4f.w);
        this.x = (float)(n3 * this.x + n4 * quat4f.x);
        this.y = (float)(n3 * this.y + n4 * quat4f.y);
        this.z = (float)(n3 * this.z + n4 * quat4f.z);
    }
    
    public final void interpolate(final Quat4f quat4f, final Quat4f quat4f2, final float n) {
        double n2 = quat4f2.x * quat4f.x + quat4f2.y * quat4f.y + quat4f2.z * quat4f.z + quat4f2.w * quat4f.w;
        if (n2 < 0.0) {
            quat4f.x = -quat4f.x;
            quat4f.y = -quat4f.y;
            quat4f.z = -quat4f.z;
        ...
        this.w = (float)(n3 * quat4f.w + n4 * quat4f2.w);
        this.x = (float)(n3 * quat4f.x + n4 * quat4f2.x);
        this.y = (float)(n3 * quat4f.y + n4 * quat4f2.y);
        this.z = (float)(n3 * quat4f.z + n4 * quat4f2.z);
    }
}
