...
extends FuncFloor {
    ...
    Object unpack(Info vi, Buffer opb) {
        ...
        for (j2 = 0; j2 < maxclass + 1; ++j2) {
            ...
            if (info.class_book[j2] < 0 || info.class_book[j2] >= vi.books) {
                info.free();
                return null;
            }
            for (k2 = 0; k2 < 1 << info.class_subs[j2]; ++k2) {
                info.class_subbook[j2][k2] = opb.read(8) - 1;
                if (info.class_subbook[j2][k2] >= -1 && info.class_subbook[j2][k2] < vi.books) continue;
                info.free();
                return null;
            }
        }
    ...
    Object look(DspState vd, InfoMode mi, Object i2) {
        ...
        for (j2 = 0; j2 < _n - 2; ++j2) {
            ...
            for (int k3 = 0; k3 < j2 + 2; ++k3) {
                ...
                if (x2 > lx && x2 < currentx) {
                    lo = k3;
                    lx = x2;
                }
                if (x2 >= hx || x2 <= currentx) continue;
                hi = k3;
                hx = x2;
            }
        ...
        if (vb.opb.read(1) == 1) {
            ...
            for (i2 = 0; i2 < info.partitions; ++i2) {
                ...
                for (int k2 = 0; k2 < cdim; ++k2) {
                    ...
                    cval >>>= csubbits;
                    if (book >= 0) {
                        fit_value[j2 + k2] = books[book].decode(vb.opb);
    ...
    int inverse2(Block vb, Object i2, Object memo, float[] out) {
        ...
        InfoFloor1 info = look.vi;
        int n2 = vb.vd.vi.blocksizes[vb.mode] / 2;
        if (memo != null) {
            int j2;
