...
public class GameSettings
{
    ...
    public static boolean isKeyDown(final KeyBinding key) {
        return key.getKeyCode() != 0 && ((key.getKeyCode() >= 0) ? Keyboard.isKeyDown(key.getKeyCode()) : Mouse.isButtonDown(key.getKeyCode() + 100));
    }
    ...
    public String getKeyBinding(final Options settingOption) {
        ...
        final String s2 = I18n.format(settingOption.getEnumString(), new Object[0]) + ": ";
        if (settingOption.getEnumFloat()) {
            ...
            return (settingOption == Options.MIPMAP_LEVELS && f1 >= 4.0) ? (s2 + Lang.get("of.general.max")) : ((settingOption == Options.SENSITIVITY) ? ((f2 == 0.0f) ? (s2 + I18n.format("options.sensitivity.min", new Object[0])) : ((f2 == 1.0f) ? (s2 + I18n.format("options.sensitivity.max", new Object[0])) : (s2 + (int)(f2 * 200.0f) + "%"))) : ((settingOption == Options.FOV) ? ((f1 == 70.0f) ? (s2 + I18n.format("options.fov.min", new Object[0])) : ((f1 == 110.0f) ? (s2 + I18n.format("options.fov.max", new Object[0])) : (s2 + (int)f1))) : ((settingOption == Options.FRAMERATE_LIMIT) ? ((f1 == settingOption.$VALUES) ? (s2 + I18n.format("options.framerateLimit.max", new Object[0])) : (s2 + (int)f1 + " fps")) : ((settingOption == Options.RENDER_CLOUDS) ? ((f1 == settingOption.valueMax) ? (s2 + I18n.format("options.cloudHeight.min", new Object[0])) : (s2 + ((int)f1 + 128))) : ((settingOption == Options.GAMMA) ? ((f2 == 0.0f) ? (s2 + I18n.format("options.gamma.min", new Object[0])) : ((f2 == 1.0f) ? (s2 + I18n.format("options.gamma.max", new Object[0])) : (s2 + "+" + (int)(f2 * 100.0f) + "%"))) : ((settingOption == Options.SATURATION) ? (s2 + (int)(f2 * 400.0f) + "%") : ((settingOption == Options.CHAT_OPACITY) ? (s2 + (int)(f2 * 90.0f + 10.0f) + "%") : ((settingOption == Options.CHAT_HEIGHT_UNFOCUSED) ? (s2 + GuiNewChat.calculateChatboxHeight(f2) + "px") : ((settingOption == Options.CHAT_HEIGHT_FOCUSED) ? (s2 + GuiNewChat.calculateChatboxHeight(f2) + "px") : ((settingOption == Options.CHAT_WIDTH) ? (s2 + GuiNewChat.calculateChatboxWidth(f2) + "px") : ((settingOption == Options.RENDER_DISTANCE) ? (s2 + (int)f1 + " chunks") : ((settingOption == Options.MIPMAP_LEVELS) ? ((f1 == 0.0f) ? (s2 + I18n.format("options.off", new Object[0])) : (s2 + (int)f1)) : (s2 + (int)(f2 * 100.0f) + "%")))))))))))));
        }
        if (settingOption.getEnumBoolean()) {
            final boolean flag = this.getOptionOrdinalValue(settingOption);
            return flag ? (s2 + I18n.format("options.on", new Object[0])) : (s2 + I18n.format("options.off", new Object[0]));
        }
        if (settingOption == Options.GUI_SCALE) {
            return (this.guiScale >= GameSettings.GUISCALES.length) ? (s2 + this.guiScale + "x") : (s2 + getTranslation(GameSettings.GUISCALES, this.guiScale));
        }
        if (settingOption == Options.CHAT_VISIBILITY) {
            return s2 + I18n.format(this.chatVisibility.getResourceKey(), new Object[0]);
        }
        if (settingOption == Options.PARTICLES) {
            return s2 + getTranslation(GameSettings.PARTICLES, this.particleSetting);
        }
        if (settingOption == Options.AMBIENT_OCCLUSION) {
            return s2 + getTranslation(GameSettings.AMBIENT_OCCLUSIONS, this.ambientOcclusion);
        }
        if (settingOption == Options.STREAM_COMPRESSION) {
            return s2 + getTranslation(GameSettings.STREAM_COMPRESSIONS, this.streamCompression);
        }
        if (settingOption == Options.STREAM_CHAT_ENABLED) {
            return s2 + getTranslation(GameSettings.STREAM_CHAT_MODES, this.streamChatEnabled);
        }
        if (settingOption == Options.STREAM_CHAT_USER_FILTER) {
            return s2 + getTranslation(GameSettings.STREAM_CHAT_FILTER_MODES, this.streamChatUserFilter);
        }
        if (settingOption == Options.STREAM_MIC_TOGGLE_BEHAVIOR) {
            return s2 + getTranslation(GameSettings.STREAM_MIC_MODES, this.streamMicToggleBehavior);
        }
        if (settingOption == Options.RENDER_CLOUDS) {
            return s2 + getTranslation(GameSettings.field_181149_aW, this.clouds);
        }
        if (settingOption != Options.GRAPHICS) {
            return s2;
        }
        if (this.fancyGraphics) {
            return s2 + I18n.format("options.graphics.fancy", new Object[0]);
        }
        final String s3 = "options.graphics.fast";
        return s2 + I18n.format("options.graphics.fast", new Object[0]);
    }
                    ...
                    while ((s = bufferedreader.readLine()) != null) {
                        try {
                            ...
                            if (astring[0].equals("incompatibleResourcePacks")) {
                                this.incompatibleResourcePacks = (List<String>)GameSettings.gson.fromJson(s.substring(s.indexOf(58) + 1), (Type)GameSettings.typeListString);
                    ...
                    if (p_getKeyBindingOF_1_ == Options.BETTER_GRASS) {
                        ...
                        if (p_getKeyBindingOF_1_ == Options.VIGNETTE) {
                            ...
                            if (p_getKeyBindingOF_1_ == Options.AA_LEVEL) {
                                ...
                                return (this.ofAaLevel == 0) ? (s + Lang.getOff() + s4) : (s + this.ofAaLevel + s4);
                            }
                            ...
                            if (p_getKeyBindingOF_1_ == Options.DYNAMIC_LIGHTS) {
                                ...
                                return s + getTranslation(GameSettings.KEYS_DYNAMIC_LIGHTS, m);
                            }
