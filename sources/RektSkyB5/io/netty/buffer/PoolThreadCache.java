...
final class PoolThreadCache {
    ...
    private static <T> SubPageMemoryRegionCache<T>[] createSubPageCaches(int cacheSize, int numCaches) {
        if (cacheSize > 0) {
            ...
            for (int i2 = 0; i2 < cache.length; ++i2) {
                cache[i2] = new SubPageMemoryRegionCache(cacheSize);
            }
            return cache;
        }
        return null;
    ...
    private static <T> NormalMemoryRegionCache<T>[] createNormalCaches(int cacheSize, int maxCachedBufferCapacity, PoolArena<T> area) {
        if (cacheSize > 0) {
            ...
            for (int i2 = 0; i2 < cache.length; ++i2) {
                cache[i2] = new NormalMemoryRegionCache(cacheSize);
            }
            return cache;
        }
        return null;
    ...
    private static int free(MemoryRegionCache<?>[] caches) {
        ...
        int numFreed = 0;
        for (MemoryRegionCache<?> c2 : caches) {
            numFreed += PoolThreadCache.free(c2);
        }
        return numFreed;
    }
    ...
    private static abstract class MemoryRegionCache<T> {
        ...
        public int free() {
            ...
            int i2 = this.head;
            while (true) {
                if (MemoryRegionCache.freeEntry(this.entries[i2])) {
                    ++numFreed;
                } else {
                    return numFreed;
                }
