...
extends StoredObject {
    ...
    public String sendTeamForScore(String score) {
        ...
        int l2 = 16;
        int i2 = Math.min(16, score.length() - 16);
        String name = score.substring(i2, i2 + l2);
        while (this.scoreTeamNames.contains(name) || this.teams.containsKey(name)) {
            --i2;
            while (score.length() - l2 - i2 > 16) {
                if (--l2 < 1) {
                    return score;
                }
                i2 = Math.min(16, score.length() - l2);
            }
            name = score.substring(i2, i2 + l2);
        }
        String prefix = score.substring(0, i2);
        String suffix = i2 + l2 >= score.length() ? "" : score.substring(i2 + l2, score.length());
