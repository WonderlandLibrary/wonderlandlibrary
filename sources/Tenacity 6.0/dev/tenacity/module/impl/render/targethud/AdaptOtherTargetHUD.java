...
public class AdaptOtherTargetHUD extends TargetHUD
{
    ...
    public void render(final float x, final float y, final float alpha, final EntityLivingBase target) {
        final CustomFont rubikBold = AdaptOtherTargetHUD.rubikFont.boldSize(18);
        final CustomFont rubikSmall = AdaptOtherTargetHUD.rubikFont.size(13);
        ...
        if (this.animation.getOutput() > 0.0f) {
            RenderUtil.drawGradientRectBordered(x + 2.5, y + 36.5, x + 1.5 + this.animation.getOutput(), y + 39.0f, 0.74, ColorUtil.applyOpacity(-16737215, alpha), ColorUtil.applyOpacity(-7405631, alpha), bg, bg);
        }
        final double armorValue = target.getTotalArmorValue() / 20.0;
        if (armorValue > 0.0) {
            RenderUtil.drawGradientRectBordered(x + 2.5, y + 32.0f, x + 1.5 + (this.getWidth() - 3.5) * armorValue, y + 34.5, 0.74, ColorUtil.applyOpacity(-16750672, alpha), ColorUtil.applyOpacity(-12986881, alpha), bg, bg);
        }
        ...
        if (target instanceof AbstractClientPlayer) {
            ...
            this.renderPlayer2D(x + 3.0f, y + 3.0f, 32.0f, 32.0f, (AbstractClientPlayer)target);
        }
        else {
        ...
        rubikSmall.drawString("Distance: " + this.DF_1.format(AdaptOtherTargetHUD.mc.thePlayer.getDistanceToEntity(target)) + "m", x + 31.0f, y + 15.0f, textColor);
    }
