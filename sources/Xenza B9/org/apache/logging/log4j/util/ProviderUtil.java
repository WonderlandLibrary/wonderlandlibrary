...
public final class ProviderUtil
{
    ...
    private ProviderUtil() {
        ServiceLoaderUtil.loadServices(Provider.class, MethodHandles.lookup(), false).filter(provider -> validVersion(provider.getVersions())).forEach(ProviderUtil.PROVIDERS::add);
        for (final LoaderUtil.UrlResource resource : LoaderUtil.findUrlResources("META-INF/log4j-provider.properties", false)) {
    ...
    protected static void loadProviders(final ClassLoader classLoader) {
        ServiceLoaderUtil.loadClassloaderServices(Provider.class, MethodHandles.lookup(), classLoader, true).filter(provider -> validVersion(provider.getVersions())).forEach(ProviderUtil.PROVIDERS::add);
    }
