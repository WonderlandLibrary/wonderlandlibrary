...
extends ViaListener {
    ...
    @Listener
    public void onInventoryClick(ClickInventoryEvent e2, @Root Player player) {
        for (SlotTransaction transaction : e2.getTransactions()) {
            if (!ArmorType.isArmor(((ItemStackSnapshot)transaction.getFinal()).getType().getId()) && !ArmorType.isArmor(((ItemStackSnapshot)e2.getCursorTransaction().getFinal()).getType().getId())) continue;
    ...
    @Listener
    public void onJoin(ClientConnectionEvent.Join e2) {
        this.sendArmorUpdate(e2.getTargetEntity());
    }

    @Listener
    public void onRespawn(RespawnPlayerEvent e2) {
        this.sendDelayedArmorUpdate(e2.getTargetEntity());
    }

    @Listener
    public void onWorldChange(DisplaceEntityEvent.Teleport e2) {
        if (!(e2.getTargetEntity() instanceof Player)) {
            return;
        }
        if (!((World)e2.getFromTransform().getExtent()).getUniqueId().equals(((World)e2.getToTransform().getExtent()).getUniqueId())) {
            this.sendArmorUpdate((Player)e2.getTargetEntity());
        }
    }
    ...
    protected int getEntityId(Player p2) {
        try {
            if (entityIdField == null) {
                entityIdField = p2.getClass().getSuperclass().getSuperclass().getSuperclass().getDeclaredField("field_145783_c");
                entityIdField.setAccessible(true);
            }
            return entityIdField.getInt(p2);
        }
        catch (Exception e2) {
