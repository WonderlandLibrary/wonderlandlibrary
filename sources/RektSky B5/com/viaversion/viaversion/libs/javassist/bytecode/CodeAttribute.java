...
import com.viaversion.viaversion.libs.javassist.bytecode.CodeAnalyzer;
    ...
    private static void shiftIndex(CodeIterator ci, int lessThan, int delta) throws BadBytecode {
        ...
        if (opcode < 79) {
            ...
            if ((var2 += delta) < 256) {
                ...
                ci.write16bit(var2, pos - 1);
                ci.write16bit(plus, pos + 1);
            }
        } else if (opcode == 169) {
            CodeAttribute.shiftIndex8(ci, index, opcode, lessThan, delta);
        } else if (opcode == 196) {
            int var3 = ci.u16bitAt(index + 2);
            if (var3 < lessThan) {
                return;
            }
            ci.write16bit(var3 += delta, index + 2);
        }
    }
