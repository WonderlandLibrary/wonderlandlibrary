...
implements Emitable {
    ...
    private boolean checkEmptyDocument() {
        ...
        if (event instanceof ScalarEvent) {
            ScalarEvent e2 = (ScalarEvent)event;
            return e2.getAnchor() == null && e2.getTag() == null && e2.getImplicit() != null && e2.getValue().length() == 0;
        }
        return false;
    }
    ...
    private ScalarAnalysis analyzeScalar(String scalar) {
        ...
        while (index < scalar.length()) {
            int nextIndex;
            boolean isLineBreak;
            int c2 = scalar.codePointAt(index);
            if (index == 0) {
                if ("#,[]{}&*!|>'\"%@`".indexOf(c2) != -1) {
                    flowIndicators = true;
                    blockIndicators = true;
                ...
                if (c2 == 35 && preceededByWhitespace) {
                    flowIndicators = true;
                    blockIndicators = true;
                }
            }
            if (isLineBreak = Constant.LINEBR.has(c2)) {
                lineBreaks = true;
            }
            if (c2 != 10 && (32 > c2 || c2 > 126)) {
                if (c2 == 133 || c2 >= 160 && c2 <= 55295 || c2 >= 57344 && c2 <= 65533 || c2 >= 65536 && c2 <= 0x10FFFF) {
            ...
            preceededByWhitespace = Constant.NULL_BL_T.has(c2) || isLineBreak;
            followedByWhitespace = true;
            if ((index += Character.charCount(c2)) + 1 >= scalar.length() || (nextIndex = index + Character.charCount(scalar.codePointAt(index))) >= scalar.length()) continue;
            ...
            if (ch == null || "\"\\\u0085\u2028\u2029\ufeff".indexOf(ch.charValue()) != -1 || ' ' > ch.charValue() || ch.charValue() > '~') {
                ...
                if (ch != null) {
                    ...
                    if (ESCAPE_REPLACEMENTS.containsKey(ch)) {
                        ...
                        String s2;
                        if (ch.charValue() <= '\u00ff') {
                            s2 = "0" + Integer.toString(ch.charValue(), 16);
                            data = "\\x" + s2.substring(s2.length() - 2);
                        } else if (ch.charValue() >= '\ud800' && ch.charValue() <= '\udbff') {
                            if (end + 1 < text.length()) {
                                Character ch2 = Character.valueOf(text.charAt(++end));
                                String s3 = "000" + Long.toHexString(Character.toCodePoint(ch.charValue(), ch2.charValue()));
                                data = "\\U" + s3.substring(s3.length() - 8);
                            } else {
                                s2 = "000" + Integer.toString(ch.charValue(), 16);
                                data = "\\u" + s2.substring(s2.length() - 4);
                            }
                        } else {
                            s2 = "000" + Integer.toString(ch.charValue(), 16);
                            data = "\\u" + s2.substring(s2.length() - 4);
