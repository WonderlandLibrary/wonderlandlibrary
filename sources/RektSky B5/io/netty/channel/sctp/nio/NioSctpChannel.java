...
implements SctpChannel {
        ...
        try {
            ...
            block4: {
                try {
                    ...
                    logger.warn("Failed to close a partially initialized sctp channel.", e22);
                }
            }
    ...
    protected SocketAddress localAddress0() {
        try {
            Iterator<SocketAddress> i2 = this.javaChannel().getAllLocalAddresses().iterator();
            if (i2.hasNext()) {
                return i2.next();
            }
        }
    ...
    protected SocketAddress remoteAddress0() {
        try {
            Iterator<SocketAddress> i2 = this.javaChannel().getRemoteAddresses().iterator();
            if (i2.hasNext()) {
                return i2.next();
            }
        }
    ...
    protected final Object filterOutboundMessage(Object msg) throws Exception {
        if (msg instanceof SctpMessage) {
            SctpMessage m2 = (SctpMessage)msg;
            ByteBuf buf = m2.content();
            if (buf.isDirect() && buf.nioBufferCount() == 1) {
                return m2;
            }
            return new SctpMessage(m2.protocolIdentifier(), m2.streamIdentifier(), this.newDirectBuffer(m2, buf));
        }
