...
extends ZlibDecoder {
    ...
    private boolean readGZIPHeader(ByteBuf in) {
        switch (this.gzipState) {
            case HEADER_START: {
                ...
                if (method != 8) {
                    throw new CompressionException("Unsupported compression method " + method + " in the GZIP header");
                }
    ...
    private boolean readGZIPFooter(ByteBuf buf) {
        ...
        if (dataLength != readLength) {
            throw new CompressionException("Number of bytes mismatch. Expected: " + dataLength + ", Got: " + readLength);
        }
        return true;
    }
    ...
    private void verifyCrc(ByteBuf in) {
        ...
        if (crcValue != readCrc) {
            throw new CompressionException("CRC value missmatch. Expected: " + crcValue + ", Got: " + readCrc);
        }
    }
