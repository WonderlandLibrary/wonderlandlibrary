...
public class MathHelper
{
    ...
    public static float sin(final float rad) {
        return MathHelper.BF_sinFull[(int)(rad * MathHelper.BF_radToIndex) & MathHelper.BF_SIN_MASK];
    }
    
    public static float cos(final float rad) {
        return sin(rad + MathHelper.BF_SIN_TO_COS);
    }
    ...
    public static double denormalizeClamp(final double lowerBnd, final double upperBnd, final double slide) {
    ...
    public static int roundUp(final int p_154354_0_, int p_154354_1_) {
        if (p_154354_1_ == 0) {
            return 0;
        }
        if (p_154354_0_ == 0) {
    ...
    public static int hsvToRGB(final float p_181758_0_, final float p_181758_1_, final float p_181758_2_) {
    ...
    static {
        ...
        BF_SIN_BITS = 12;
        BF_SIN_MASK = ~(-1 << MathHelper.BF_SIN_BITS);
        BF_SIN_COUNT = MathHelper.BF_SIN_MASK + 1;
        BF_radFull = 6.2831855f;
        BF_radToIndex = MathHelper.BF_SIN_COUNT / MathHelper.BF_radFull;
        BF_sinFull = new float[MathHelper.BF_SIN_COUNT];
        for (int i = 0; i < MathHelper.BF_SIN_COUNT; ++i) {
            MathHelper.BF_sinFull[i] = (float)Math.sin((i + Math.min(1, i % (MathHelper.BF_SIN_COUNT / 4)) * 0.5) / MathHelper.BF_SIN_COUNT * MathHelper.BF_radFull);
        }
