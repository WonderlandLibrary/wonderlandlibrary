...
public class GuiOverlayDebug extends Gui
{
    ...
    protected void renderDebugInfoLeft() {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (!Strings.isNullOrEmpty(s)) {
                ...
                final int k = (int)this.fontRenderer.getStringWidth(s);
                final int l = 2;
                final int i2 = 2 + j * i;
                Gui.drawRect(1.0, i2 - 1, 2 + k + 1, i2 + j - 1, -1873784752);
    ...
    protected List<String> getDebugInfoRight() {
        ...
        final long l = j - k;
        final List<String> list = (List<String>)Lists.newArrayList((Object[])new String[] { String.format("Java: %s %dbit", System.getProperty("java.version"), this.mc.isJava64bit() ? 64 : 32), String.format("Mem: % 2d%% %03d/%03dMB", l * 100L / i, bytesToMb(l), bytesToMb(i)), String.format("Allocated: % 2d%% %03dMB", j * 100L / i, bytesToMb(j)), "", String.format("CPU: %s", OpenGlHelper.getCpu()), "", String.format("Display: %dx%d (%s)", Display.getWidth(), Display.getHeight(), GL11.glGetString(7936)), GL11.glGetString(7937), GL11.glGetString(7938) });
