...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    protected void channelRead0(final ChannelHandlerContext p_channelRead0_1_, final Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                final PacketReceiveEvent e = new PacketReceiveEvent(p_channelRead0_2_);
    ...
    public void sendPacket(final Packet packetIn) {
        this.sendPacket(packetIn, false);
    }
    
    public void sendPacket(Packet packetIn, final boolean silent) {
        if (this.isChannelOpen()) {
            if (!silent) {
                final PacketSendEvent e = new PacketSendEvent(packetIn);
                Tenacity.INSTANCE.getEventProtocol().handleEvent(e);
                if (e.isCancelled()) {
                    return;
                }
    ...
    private void dispatchPacket(final Packet inPacket, final GenericFutureListener<? extends Future<? super Void>>[] futureListeners) {
        ...
        if (this.channel.eventLoop().inEventLoop()) {
            ...
            final EnumConnectionState connectionState;
            final ChannelFuture channelfuture2;
            this.channel.eventLoop().execute(() -> {
                if (connectionState != enumconnectionstate2) {
                    this.setConnectionState(connectionState);
                }
                channelfuture2 = this.channel.writeAndFlush((Object)inPacket);
                if (futureListeners != null) {
                    channelfuture2.addListeners((GenericFutureListener[])futureListeners);
                }
        ...
        ((Bootstrap)((Bootstrap)((Bootstrap)new Bootstrap().group((EventLoopGroup)lazyloadbase.getValue())).handler((ChannelHandler)new ChannelInitializer<Channel>() {
            protected void initChannel(final Channel p_initChannel_1_) {
                ...
                if (p_initChannel_1_ instanceof SocketChannel && ViaMCP.getInstance().getVersion() != 47) {
                    final UserConnection user = (UserConnection)new UserConnectionImpl(p_initChannel_1_, true);
