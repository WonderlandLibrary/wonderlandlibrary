...
implements ViaPlatform<Player> {
    ...
    public ViaCommandSender[] getOnlinePlayers() {
        ...
        for (Player player : this.game.getServer().getOnlinePlayers()) {
            array[i2++] = new SpongeCommandSender((CommandSource)player);
        }
        return array;
    }
    ...
    public JsonObject getDump() {
        ...
        for (PluginContainer p2 : this.game.getPluginManager().getPlugins()) {
            plugins.add(new PluginInfo(true, p2.getName(), p2.getVersion().orElse("Unknown Version"), p2.getInstance().isPresent() ? p2.getInstance().get().getClass().getCanonicalName() : "Unknown", p2.getAuthors()));
        }
