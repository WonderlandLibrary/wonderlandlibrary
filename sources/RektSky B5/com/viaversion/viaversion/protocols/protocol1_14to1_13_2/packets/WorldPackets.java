                ...
                this.handler(new PacketHandler(){
                    ...
                    public void handle(PacketWrapper wrapper) throws Exception {
                            ...
                            for (int x2 = 0; x2 < 16; ++x2) {
                                for (int y2 = 0; y2 < 16; ++y2) {
                                    for (int z2 = 0; z2 < 16; ++z2) {
                                        int id = section.getFlatBlock(x2, y2, z2);
                                        if (id != air && id != voidAir && id != caveAir) {
                                            ++nonAirBlockCount;
                                            worldSurface[x2 + z2 * 16] = y2 + s2 * 16 + 1;
                                        }
                                        if (protocol.getMappingData().getMotionBlocking().contains(id)) {
                                            motionBlocking[x2 + z2 * 16] = y2 + s2 * 16 + 1;
                                        }
                        ...
                        for (int i3 = 0; i3 < chunk.getSections().length; ++i3) {
                            ChunkSection sec = chunk.getSections()[i3];
                            if (sec == null) continue;
                            if (!chunk.isFullChunk() && sec.getLight().hasSkyLight()) {
                                skyLightMask |= 1 << i3 + 1;
                            }
                            blockLightMask |= 1 << i3 + 1;
                        }
