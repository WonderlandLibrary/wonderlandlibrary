                        ...
                        if (channel.equalsIgnoreCase("MC|TrList")) {
                            ...
                            if (channel.equals("minecraft:register") || channel.equals("minecraft:unregister")) {
                                ...
                                for (int i3 = 0; i3 < channels.length; ++i3) {
                                    String rewritten = InventoryPackets.getNewPluginChannelId(channels[i3]);
                                    if (rewritten != null) {
                                        rewrittenChannels.add(rewritten);
                                        continue;
                                    }
                                    if (Via.getConfig().isSuppressConversionWarnings() && !Via.getManager().isDebug()) continue;
                                    Via.getPlatform().getLogger().warning("Ignoring plugin channel in outgoing REGISTER: " + channels[i3]);
                                }
                    ...
                    public void handle(PacketWrapper wrapper) throws Exception {
                        ...
                        if (channel.equals("REGISTER") || channel.equals("UNREGISTER")) {
                            ...
                            for (int i2 = 0; i2 < channels.length; ++i2) {
                                String rewritten = InventoryPackets.getOldPluginChannelId(channels[i2]);
                                if (rewritten != null) {
                                    rewrittenChannels.add(rewritten);
                                    continue;
                                }
                                if (Via.getConfig().isSuppressConversionWarnings() && !Via.getManager().isDebug()) continue;
                                Via.getPlatform().getLogger().warning("Ignoring plugin channel in incoming REGISTER: " + channels[i2]);
                            }
                            wrapper.write(Type.REMAINING_BYTES, Joiner.on('\u0000').join(rewrittenChannels).getBytes(StandardCharsets.UTF_8));
                        }
                        wrapper.set(Type.STRING, 0, channel);
