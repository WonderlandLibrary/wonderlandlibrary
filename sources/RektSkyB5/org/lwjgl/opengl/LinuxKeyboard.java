...
final class LinuxKeyboard {
    ...
    LinuxKeyboard(long display, long window) {
        ...
        if (modifier_map != 0L) {
            ...
            for (int i2 = 0; i2 < 8; ++i2) {
                block7: for (int j2 = 0; j2 < max_keypermod; ++j2) {
                    int key_code = LinuxKeyboard.lookupModifierMap(modifier_map, i2 * max_keypermod + j2);
                    ...
                    switch (key_sym) {
                        ...
                        case 65510: {
                            if (i2 != 1 || tmp_caps_lock_mask != 0) continue block7;
    ...
    private int lookupStringISO88591(long event_ptr, int[] translation_buffer) {
        ...
        for (int i2 = 0; i2 < num_chars; ++i2) {
            translation_buffer[i2] = this.native_translation_buffer.get(i2) & 0xFF;
        }
        return num_chars;
    }
    ...
    private int lookupStringUnicode(long event_ptr, int[] translation_buffer) {
        ...
        int i2 = 0;
        while (this.char_buffer.hasRemaining() && i2 < translation_buffer.length) {
    ...
    private void translateEvent(long event_ptr, int keycode, byte key_state, long nanos, boolean repeat) {
        ...
        if (num_chars > 0) {
            ...
            for (int i2 = 1; i2 < num_chars; ++i2) {
                ch = this.temp_translation_buffer[i2];
