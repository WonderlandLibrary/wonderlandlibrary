...
public final class Profiler {
    ...
    private void printSectionRow(PrettyPrinter printer, int colCount, int[] columns, Section section, boolean group) {
        ...
        Object[] values2 = new Object[colCount];
        int col = 1;
        values2[0] = group ? (isDelegate ? "  > " + section.getBaseName() : section.getName()) : (isDelegate ? "+ " : "  ") + section.getName();
        for (long time : times = section.getTimes()) {
            if (col == columns[1]) {
                values2[col++] = section.getTotalTime() + " ms";
            }
            if (col < columns[2] || col >= values2.length) continue;
            values2[col++] = time + " ms";
        }
        values2[columns[3]] = section.getTotalCount();
        values2[columns[4]] = new DecimalFormat("   ###0.000 ms").format(section.getTotalAverageTime());
