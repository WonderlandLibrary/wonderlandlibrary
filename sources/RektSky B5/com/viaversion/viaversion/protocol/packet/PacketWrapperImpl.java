...
implements PacketWrapper {
    ...
    public <T> T get(Type<T> type, int index) throws Exception {
        ...
        ArrayIndexOutOfBoundsException e2 = new ArrayIndexOutOfBoundsException("Could not find type " + type.getTypeName() + " at " + index);
        throw new InformativeException(e2).set("Type", type.getTypeName()).set("Index", index).set("Packet ID", this.getId()).set("Packet Type", this.packetType).set("Data", this.packetValues);
    }
    ...
    public <T> void set(Type<T> type, int index, T value) throws Exception {
        ...
        ArrayIndexOutOfBoundsException e2 = new ArrayIndexOutOfBoundsException("Could not find type " + type.getTypeName() + " at " + index);
        throw new InformativeException(e2).set("Type", type.getTypeName()).set("Index", index).set("Packet ID", this.getId()).set("Packet Type", this.packetType);
    }
    ...
    public <T> T read(Type<T> type) throws Exception {
        ...
        IOException e3 = new IOException("Unable to read type " + type.getTypeName() + ", found " + read.key().getTypeName());
        throw new InformativeException(e3).set("Type", type.getTypeName()).set("Packet ID", this.getId()).set("Packet Type", this.packetType).set("Data", this.packetValues);
    }
    ...
    public void writeToBuffer(ByteBuf buffer) throws Exception {
        ...
        for (Pair<Type, Object> packetValue : this.packetValues) {
            try {
                ...
                throw new InformativeException(e2).set("Index", index).set("Type", packetValue.key().getTypeName()).set("Packet ID", this.getId()).set("Packet Type", this.packetType).set("Data", this.packetValues);
            }
            ++index;
        }
    ...
    private void send0(Class<? extends Protocol> protocol, boolean skipCurrentPipeline, boolean currentThread) throws Exception {
        block5: {
            ...
            try {
                ...
                if (PipelineUtil.containsCause(e2, CancelException.class)) break block5;
                throw e2;
            }
        }
    }
    ...
    private ByteBuf constructPacket(Class<? extends Protocol> packetProtocol, boolean skipCurrentPipeline, Direction direction) throws Exception {
        ...
        for (int i2 = 0; i2 < protocols.length; ++i2) {
            if (protocols[i2].getClass() != packetProtocol) continue;
            index = i2;
            break;
        }
        if (index == -1) {
    ...
    private PacketWrapperImpl apply(Direction direction, State state, int index, Protocol[] pipeline, boolean reverse) throws Exception {
        if (reverse) {
            ...
            for (int i3 = index; i3 < pipeline.length; ++i3) {
                pipeline[i3].transform(direction, state, this);
                this.resetReader();
            }
        }
        return this;
