...
public class PatternTokenizer {
    ...
    public String quoteLiteral(String string) {
        ...
        for (int i2 = 0; i2 < string.length(); i2 += UTF16.getCharCount(cp)) {
            cp = UTF16.charAt(string, i2);
    ...
    public int next(StringBuffer buffer) {
        ...
        int hexCount = 0;
        int hexValue = 0;
        block21: for (int i2 = this.start; i2 < this.limit; i2 += UTF16.getCharCount(cp)) {
            cp = UTF16.charAt(this.pattern, i2);
            switch (quoteStatus) {
                case 3: {
                    switch (cp) {
                        case 117: {
            ...
            if (this.syntaxCharacters.contains(cp)) {
                if (status == 5) {
                    UTF16.append(buffer, cp);
                    this.start = i2 + UTF16.getCharCount(cp);
                    return 1;
                }
                this.start = i2;
                return status;
