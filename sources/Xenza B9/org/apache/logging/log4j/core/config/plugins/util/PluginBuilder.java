...
public class PluginBuilder implements Builder<Object>
{
    ...
    public Object build() {
        ...
        try {
            ...
            PluginBuilder.LOGGER.error("Could not create plugin of type {} for element {}: {}", this.clazz, this.node.getName(), ((t instanceof InvocationTargetException) ? ((InvocationTargetException)t).getCause() : t).toString(), t);
        }
        try {
            ...
            PluginBuilder.LOGGER.error("Unable to invoke factory method in {} for element {}: {}", this.clazz, this.node.getName(), ((t instanceof InvocationTargetException) ? ((InvocationTargetException)t).getCause() : t).toString(), t);
            return null;
        }
    }
    ...
    private void injectFields(final Builder<?> builder) throws IllegalAccessException {
        ...
        AccessibleObject.setAccessible(fields.toArray(PluginBuilder.EMPTY_FIELD_ARRAY), true);
        ...
        for (final Field field : fields) {
            ...
            for (final Annotation a : annotations) {
                if (!(a instanceof PluginAliases)) {
                    ...
                    if (visitor != null) {
                        final Object value = visitor.setAliases(aliases).setAnnotation(a).setConversionType(field.getType()).setStrSubstitutor((this.event == null) ? this.configuration.getConfigurationStrSubstitutor() : this.configuration.getStrSubstitutor()).setMember(field).visit(this.configuration, this.node, this.event, log);
                        if (value != null) {
    ...
    public static boolean validateFields(final Builder<?> builder, final String errorPrefix) {
        ...
        try {
            final String reason = validateFields(builder, fields);
            if (!reason.isEmpty()) {
                PluginBuilder.LOGGER.error("{}: {}", errorPrefix, reason);
                return false;
            }
        }
        catch (final IllegalAccessException e) {
            PluginBuilder.LOGGER.error("{}: {}", errorPrefix, e.getMessage(), e);
            return false;
        }
        return true;
    }
        ...
        for (int i = 0; i < annotations.length; ++i) {
            ...
            for (final Annotation a : annotations[i]) {
                if (!(a instanceof PluginAliases)) {
                    final PluginVisitor<? extends Annotation> visitor = PluginVisitors.findVisitor(a.annotationType());
                    if (visitor != null) {
                        final Object value = visitor.setAliases(aliases).setAnnotation(a).setConversionType(types[i]).setStrSubstitutor((this.event == null) ? this.configuration.getConfigurationStrSubstitutor() : this.configuration.getStrSubstitutor()).setMember(factory).visit(this.configuration, this.node, this.event, log);
                        if (value != null) {
                            args[i] = value;
                        }
                    }
