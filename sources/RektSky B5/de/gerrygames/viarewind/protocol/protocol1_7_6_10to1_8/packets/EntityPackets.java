        ...
        protocol.registerClientbound(ClientboundPackets1_8.ENTITY_POSITION, new PacketRemapper(){
            ...
            public void registerMap() {
                ...
                this.handler(packetWrapper -> {
                    ...
                    if (replacement != null) {
                        packetWrapper.cancel();
                        byte x2 = packetWrapper.get(Type.BYTE, 0);
                        byte y2 = packetWrapper.get(Type.BYTE, 1);
                        byte z2 = packetWrapper.get(Type.BYTE, 2);
                        replacement.relMove((double)x2 / 32.0, (double)y2 / 32.0, (double)z2 / 32.0);
                    }
                });
            }
        });
            ...
            public void registerMap() {
                ...
                this.handler(packetWrapper -> {
                    ...
                    if (type == Entity1_10Types.EntityType.MINECART_ABSTRACT) {
                        int y2 = packetWrapper.get(Type.INT, 2);
                        packetWrapper.set(Type.INT, 2, y2 += 12);
                    }
                });
                this.handler(packetWrapper -> {
                    int entityId = packetWrapper.get(Type.INT, 0);
                    ...
                    if (replacement != null) {
                        packetWrapper.cancel();
                        int x2 = packetWrapper.get(Type.INT, 1);
                        int y2 = packetWrapper.get(Type.INT, 2);
                        int z2 = packetWrapper.get(Type.INT, 3);
                        byte yaw = packetWrapper.get(Type.BYTE, 0);
                        byte pitch = packetWrapper.get(Type.BYTE, 1);
