...
extends ViaSpongeListener {
    ...
    @Listener
    public void placeBlock(ChangeBlockEvent.Place e2, @Root Player player) {
        if (this.isOnPipe(player.getUniqueId())) {
            Location loc = (Location)((BlockSnapshot)((Transaction)e2.getTransactions().get(0)).getFinal()).getLocation().get();
