    ...
    private static Map<String, List<Rule>> parseRules(Scanner scanner, final String location) {
        ...
        while (scanner.hasNextLine()) {
            ...
            try {
                ...
                Rule r2 = new Rule(pat, lCon, rCon, ph){
    ...
    static {
        for (NameType s2 : NameType.values()) {
            ...
            for (RuleType rt : RuleType.values()) {
                ...
                for (String l2 : ls.getLanguages()) {
                    Scanner scanner = Rule.createScanner(s2, rt, l2);
                    try {
                        rs.put(l2, Rule.parseRules(scanner, Rule.createResourceName(s2, rt, l2)));
                    }
                    catch (IllegalStateException e2) {
                        throw new IllegalStateException("Problem processing " + Rule.createResourceName(s2, rt, l2), e2);
                    }
                    finally {
                        scanner.close();
                    }
                ...
                if (!rt.equals((Object)RuleType.RULES)) {
                    Scanner scanner = Rule.createScanner(s2, rt, "common");
                    try {
                        rs.put("common", Rule.parseRules(scanner, Rule.createResourceName(s2, rt, "common")));
                    }
                    finally {
                        scanner.close();
                    }
