...
public class ScriptManager implements FileWatcher
{
    ...
    public ScriptManager(final Configuration configuration, final WatchManager watchManager, final String scriptLanguages) {
        ...
        this.allowedLanguages = Arrays.stream(Strings.splitList(scriptLanguages)).map((Function<? super String, ?>)String::toLowerCase).collect((Collector<? super Object, ?, Set<String>>)Collectors.toSet());
        if (ScriptManager.logger.isDebugEnabled()) {
            ...
            for (final ScriptEngineFactory factory2 : factories) {
                for (final String name2 : factory2.getNames()) {
                    if (this.allowedLanguages.contains(name2.toLowerCase(Locale.ROOT))) {
                        if (names2.length() > 0) {
                            names2.append(", ");
                        }
                        names2.append(name2);
    ...
    public boolean addScript(final AbstractScript script) {
        if (!this.allowedLanguages.contains(script.getLanguage().toLowerCase(Locale.ROOT))) {
            ScriptManager.logger.error("Unable to add script {}, {} has not been configured as an allowed language", script.getName(), script.getLanguage());
            return false;
        }
