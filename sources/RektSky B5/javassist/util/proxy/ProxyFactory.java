...
public class ProxyFactory {
    ...
    private ClassFile make() throws CannotCompileException {
        ...
        int s2 = this.overrideMethods(cf, pool, this.classname, forwarders);
        ProxyFactory.addClassInitializer(cf, pool, this.classname, s2, forwarders);
    ...
    private int overrideMethods(ClassFile cf, ConstPool cp, String className, List<Find2MethodsArgs> forwarders) throws CannotCompileException {
        ...
        int index = 0;
        while (it.hasNext()) {
            Map.Entry<String, Method> e2 = it.next();
            if ((ClassFile.MAJOR_VERSION < 49 || !ProxyFactory.isBridge(e2.getValue())) && this.testBit(this.signature, index)) {
                this.override(className, e2.getValue(), prefix, index, ProxyFactory.keyToDesc(e2.getKey(), e2.getValue()), cf, cp, forwarders);
            }
            ++index;
        }
        return index;
    ...
    private void makeConstructors(String thisClassName, ClassFile cf, ConstPool cp, String classname) throws CannotCompileException {
        ...
        for (int i2 = 0; i2 < cons.length; ++i2) {
            ...
            MethodInfo m2 = ProxyFactory.makeConstructor(thisClassName, c2, cp, this.superClass, doHandlerInit);
            cf.addMethod(m2);
        }
    }
