...
extends InventoryQuickMoveProvider {
    ...
    public Object buildWindowClickPacket(Player p2, ItemTransaction storage) {
        int protocolId;
        InventoryType tinvtype;
        if (!this.supported) {
            return null;
        }
        InventoryView inv = p2.getOpenInventory();
        ...
        try {
            ...
            Via.getPlatform().getLogger().log(Level.SEVERE, "Failed to create a window click packet. Please report this issue to the ViaVersion Github: " + e2.getMessage(), e2);
        }
        return packet;
    }

    public boolean sendPacketToServer(Player p2, Object packet) {
        if (packet == null) {
            return true;
        }
        try {
            Object entityPlayer = this.craftPlayerHandle.invoke(p2, new Object[0]);
    ...
    private void setupReflection() {
        ...
        try {
            ...
            throw new RuntimeException("Couldn't find required inventory classes", e2);
        }
        try {
        ...
        try {
            ...
            throw new RuntimeException("Couldn't find Player Connection", e4);
        }
        try {
            this.packetMethod = NMSUtil.nms("PlayerConnection").getDeclaredMethod("a", this.windowClickPacketClass);
        }
        catch (ClassNotFoundException | NoSuchMethodException e5) {
            throw new RuntimeException("Couldn't find CraftPlayer", e5);
        }
    }
