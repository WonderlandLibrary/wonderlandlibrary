...
implements Tickable {
    ...
    private void sendPackets() {
        ...
        for (Side side : Side.values()) {
            ...
            if (side.modX != 0) {
                ...
                center = this.z;
                d2 = Math.abs(this.x + radius * (double)side.modX - position.getPosX());
            } else {
                center = this.x;
                pos = position.getPosX();
                d2 = Math.abs(this.z + radius * (double)side.modZ - position.getPosZ());
            }
            if (d2 >= 16.0) continue;
            double r2 = Math.sqrt(256.0 - d2 * d2);
            double minH = Math.ceil(pos - r2);
            double maxH = Math.floor(pos + r2);
            double minV = Math.ceil(position.getPosY() - r2);
            double maxV = Math.floor(position.getPosY() + r2);
            ...
            int a2 = (int)Math.floor((maxH - minH) * (maxV - minV) * 0.5);
            double b2 = 2.5;
            ...
            particles.write(Type.FLOAT, Float.valueOf((float)(side.modX != 0 ? 0.0 : (maxH - minH) / b2)));
            particles.write(Type.FLOAT, Float.valueOf((float)((maxV - minV) / b2)));
            particles.write(Type.FLOAT, Float.valueOf((float)(side.modX == 0 ? 0.0 : (maxH - minH) / b2)));
            particles.write(Type.FLOAT, Float.valueOf(0.0f));
            particles.write(Type.INT, a2);
    ...
    public void init(double x2, double z2, double oldDiameter, double newDiameter, long lerpTime, int portalTeleportBoundary, int warningTime, int warningBlocks) {
        this.x = x2;
        this.z = z2;
