...
public class ShadersTex
{
    ...
    public static void uploadTexSubForLoadAtlas(final int[][] data, final int width, final int height, final int xoffset, final int yoffset, final boolean linear, final boolean clamp) {
        ...
        if (Shaders.configNormalMap) {
            final int[][] aint = readImageAndMipmaps(ShadersTex.iconName + "_n", width, height, data.length, flag, -8421377);
        ...
        if (Shaders.configSpecularMap) {
            final int[][] aint2 = readImageAndMipmaps(ShadersTex.iconName + "_s", width, height, data.length, flag, 0);
