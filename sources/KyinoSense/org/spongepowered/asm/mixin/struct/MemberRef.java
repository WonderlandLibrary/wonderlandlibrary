...
public abstract class MemberRef {
    ...
    static int opcodeFromTag(int tag2) {
        return tag2 >= 0 && tag2 < H_OPCODES.length ? H_OPCODES[tag2] : 0;
    }

    static int tagFromOpcode(int opcode) {
        for (int tag2 = 1; tag2 < H_OPCODES.length; ++tag2) {
            if (H_OPCODES[tag2] != opcode) continue;
            return tag2;
        }
        return 0;
    }
    ...
    extends MemberRef {
        ...
        public void setOpcode(int opcode) {
            int tag2 = MemberRef.tagFromOpcode(opcode);
            if (tag2 == 0) {
                throw new MixinTransformerError("Invalid opcode " + Bytecode.getOpcodeName(opcode) + " for method handle " + this.handle + ".");
            }
            boolean itf = tag2 == 9;
            this.handle = new org.spongepowered.asm.lib.Handle(tag2, this.handle.getOwner(), this.handle.getName(), this.handle.getDesc(), itf);
        }
