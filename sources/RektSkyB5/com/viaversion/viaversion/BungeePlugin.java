...
Listener {
    ...
    public ViaCommandSender[] getOnlinePlayers() {
        ...
        for (ProxiedPlayer player : players) {
            array[i2++] = new BungeeCommandSender((CommandSender)player);
        }
        return array;
    }
    ...
    public JsonObject getDump() {
        ...
        for (Plugin p2 : ProxyServer.getInstance().getPluginManager().getPlugins()) {
            plugins.add(new PluginInfo(true, p2.getDescription().getName(), p2.getDescription().getVersion(), p2.getDescription().getMain(), Collections.singletonList(p2.getDescription().getAuthor())));
        }
