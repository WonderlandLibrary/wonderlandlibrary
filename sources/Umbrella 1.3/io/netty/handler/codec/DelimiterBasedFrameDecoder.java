...
extends ByteToMessageDecoder {
    ...
    private void fail(ChannelHandlerContext ctx, long frameLength) {
        if (frameLength > 0L) {
            ctx.fireExceptionCaught(new TooLongFrameException("frame length exceeds " + this.maxFrameLength + ": " + frameLength + " - discarded"));
        } else {
            ctx.fireExceptionCaught(new TooLongFrameException("frame length exceeds " + this.maxFrameLength + " - discarding"));
        }
    }
