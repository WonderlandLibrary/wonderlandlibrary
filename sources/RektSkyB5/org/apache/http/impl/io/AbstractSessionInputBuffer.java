...
BufferInfo {
    ...
    protected int fillBuffer() throws IOException {
        ...
        if ((l2 = this.instream.read(this.buffer, off = this.bufferlen, len = this.buffer.length - off)) == -1) {
            return -1;
        }
    ...
    public int read(byte[] b2, int off, int len) throws IOException {
        ...
        if (len > this.minChunkLimit) {
            int read = this.instream.read(b2, off, len);
            if (read > 0) {
