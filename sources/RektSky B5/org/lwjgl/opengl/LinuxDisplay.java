...
implements DisplayImplementation {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public void createWindow(DrawableLWJGL drawable, DisplayMode mode, Canvas parent, int x2, int y2) throws LWJGLException {
        LinuxDisplay.lockAWT();
        try {
            LinuxDisplay.incDisplay();
            try {
                ...
                try {
                    ...
                    if (mode.isFullscreenCapable() && this.current_displaymode_extension == 10) {
                        ...
                        x2 = primaryScreen.xPos;
                        y2 = primaryScreen.yPos;
                    }
                    current_window = LinuxDisplay.nCreateWindow(LinuxDisplay.getDisplay(), LinuxDisplay.getDefaultScreen(), handle, mode, current_window_mode, x2, y2, undecorated, this.parent_window, this.resizable);
    ...
    private void processEvents() {
        while (LinuxEvent.getPending(LinuxDisplay.getDisplay()) > 0) {
            ...
            switch (this.event_buffer.getType()) {
                ...
                case 22: {
                    int x2 = LinuxDisplay.nGetX(LinuxDisplay.getDisplay(), LinuxDisplay.getWindow());
                    int y2 = LinuxDisplay.nGetY(LinuxDisplay.getDisplay(), LinuxDisplay.getWindow());
        ...
        try {
            ...
            if (this.current_displaymode_extension == 10) {
                ...
                for (int i2 = 0; i2 < modes.length; ++i2) {
                    modes[i2] = new DisplayMode(resolutions[i2].width, resolutions[i2].height, bpp, resolutions[i2].freq);
                }
    ...
    private boolean isParentWindowActive(long window) {
        try {
            ...
            long w2 = current_window;
            while (w2 != 0L) {
                w2 = LinuxDisplay.getParentWindow(LinuxDisplay.getDisplay(), w2);
                if (w2 != parent_window) continue;
                this.parent_proxy_focus_window = window;
                return true;
            }
        }
        catch (LWJGLException e2) {
            LWJGLUtil.log("Failed to detect if parent window is active: " + e2.getMessage());
            return true;
        }
        return false;
    }
        ...
        for (ByteBuffer icon : icons) {
            ...
            for (int y2 = 0; y2 < dimension; ++y2) {
                for (int x2 = 0; x2 < dimension; ++x2) {
                    byte r2 = icon.get(x2 * 4 + y2 * dimension * 4);
                    byte g2 = icon.get(x2 * 4 + y2 * dimension * 4 + 1);
                    byte b2 = icon.get(x2 * 4 + y2 * dimension * 4 + 2);
                    byte a2 = icon.get(x2 * 4 + y2 * dimension * 4 + 3);
                    icon_argb.put(a2);
                    icon_argb.put(r2);
                    icon_argb.put(g2);
                    icon_argb.put(b2);
            ...
            block10: {
                ...
                try {
                    ...
                    try {
                        ...
                        LWJGLUtil.log("Failed to set display icon: " + e2);
                        int n3 = 0;
                        return n3;
                    }
                }
            ...
            AccessController.doPrivileged(new PrivilegedAction<Object>(){
                ...
                public Object run() {
                    try {
                        ...
                        LWJGLUtil.log("Failed to change Compiz Legacy Fullscreen Support. Reason: " + e2.getMessage());
                    }
                    return null;
                }
            });
        ...
        private static List<String> run(String ... command) throws LWJGLException {
            ...
            try {
                ...
                try {
                    ...
                    throw new LWJGLException("Process interrupted.", e2);
                }
                ...
                throw new LWJGLException("Process failed.", e3);
            }
            return output;
        }
