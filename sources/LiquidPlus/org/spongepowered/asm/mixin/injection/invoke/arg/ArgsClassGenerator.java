...
implements IClassGenerator {
    ...
    private byte[] generateClass(String name, String desc) {
        ...
        this.generateCtor(ref, desc, args2, visitor);
        this.generateToString(ref, desc, args2, visitor);
        this.generateFactory(ref, desc, args2, visitor);
        this.generateSetters(ref, desc, args2, visitor);
        this.generateGetters(ref, desc, args2, visitor);
        visitor.visitEnd();
        return writer.toByteArray();
    }

    private void generateCtor(String ref, String desc, Type[] args2, ClassVisitor writer) {
    ...
    private void generateToString(String ref, String desc, Type[] args2, ClassVisitor writer) {
        MethodVisitor toString2 = writer.visitMethod(1, "toString", "()Ljava/lang/String;", null, null);
        toString2.visitCode();
        toString2.visitLdcInsn("Args" + ArgsClassGenerator.getSignature(args2));
        toString2.visitInsn(176);
        toString2.visitMaxs(1, 1);
        toString2.visitEnd();
    }

    private void generateFactory(String ref, String desc, Type[] args2, ClassVisitor writer) {
        ...
        of.visitInsn(89);
        of.visitConstant((byte)args2.length);
        ...
        of.visitInsn(176);
        of.visitMaxs(6, Bytecode.getArgsSize(args2));
        of.visitEnd();
    }

    private void generateGetters(String ref, String desc, Type[] args2, ClassVisitor writer) {
        byte argIndex = 0;
        for (Type arg : args2) {
    ...
    private void generateSetters(String ref, String desc, Type[] args2, ClassVisitor writer) {
        this.generateIndexedSetter(ref, desc, args2, writer);
        this.generateMultiSetter(ref, desc, args2, writer);
    }

    private void generateIndexedSetter(String ref, String desc, Type[] args2, ClassVisitor writer) {
        int index;
    ...
    private void generateMultiSetter(String ref, String desc, Type[] args2, ClassVisitor writer) {
        ...
        set.visitInsn(190);
        set.visitInsn(89);
        set.visitConstant((byte)args2.length);
        ...
        set.visitInsn(88);
        set.visitConstant((byte)args2.length);
        set.visitLdcInsn(ArgsClassGenerator.getSignature(args2));
        ...
        set.visitVarInsn(25, 0);
        set.visitFieldInsn(180, ref, VALUES_FIELD, OBJECT_ARRAY);
        for (byte index = 0; index < args2.length; index = (byte)(index + 1)) {
            set.visitInsn(89);
    ...
    private static String getSignature(Type[] args2) {
        return new SignaturePrinter("", null, args2).setFullyQualified(true).getFormattedArgs();
    }
}
