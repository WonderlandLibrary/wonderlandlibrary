...
public class MappedObjectTransformer {
    ...
    static byte[] transformMappedAPI(String className, byte[] bytecode) {
        ...
        ClassWriter cw = new ClassWriter(2){
            ...
            protected String getCommonSuperClass(String a2, String b2) {
                if (is_currently_computing_frames && !a2.startsWith("java/") || !b2.startsWith("java/")) {
                    return "java/lang/Object";
                }
                return super.getCommonSuperClass(a2, b2);
            }
        };
    ...
    static int transformMethodCall(InsnList instructions, int i2, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, MethodInsnNode methodInsn, MappedSubtypeInfo mappedType, Map<Integer, MappedSubtypeInfo> arrayVars) {
        switch (methodInsn.getOpcode()) {
            case 182: {
                ...
                if ("dup".equals(methodInsn.name) && methodInsn.desc.equals("()L" + MAPPED_OBJECT_JVM + ";")) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, (AbstractInsnNode)methodInsn, MappedObjectTransformer.generateDupInstructions(methodInsn));
                    break;
                }
                if ("slice".equals(methodInsn.name) && methodInsn.desc.equals("()L" + MAPPED_OBJECT_JVM + ";")) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, (AbstractInsnNode)methodInsn, MappedObjectTransformer.generateSliceInstructions(methodInsn));
                    break;
                }
                if ("runViewConstructor".equals(methodInsn.name) && "()V".equals(methodInsn.desc)) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, (AbstractInsnNode)methodInsn, MappedObjectTransformer.generateRunViewConstructorInstructions(methodInsn));
                    break;
                }
                if ("copyTo".equals(methodInsn.name) && methodInsn.desc.equals("(L" + MAPPED_OBJECT_JVM + ";)V")) {
                    i2 = MappedObjectTransformer.replace(instructions, i2, (AbstractInsnNode)methodInsn, MappedObjectTransformer.generateCopyToInstructions(mappedType));
                    break;
                }
                if (!"copyRange".equals(methodInsn.name) || !methodInsn.desc.equals("(L" + MAPPED_OBJECT_JVM + ";I)V")) break;
                i2 = MappedObjectTransformer.replace(instructions, i2, (AbstractInsnNode)methodInsn, MappedObjectTransformer.generateCopyRangeInstructions(mappedType));
                break;
            }
            case 183: {
            ...
            case 184: {
                ...
                i2 = MappedObjectTransformer.replace(instructions, i2, (AbstractInsnNode)methodInsn, MappedObjectTransformer.generateMapInstructions(mappedType, methodInsn.owner, isMapDirectMethod, isMallocMethod));
            }
        }
        return i2;
    }
    ...
    static int transformArrayAccess(InsnList instructions, int i2, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, VarInsnNode loadInsn, MappedSubtypeInfo mappedSubtype, int var2) {
    ...
    static int replace(InsnList instructions, int i2, AbstractInsnNode location, InsnList list) {
        int size = list.size();
    ...
    extends ClassAdapter {
                ...
                return new FieldNode(access, name, desc, signature, value){
                    ...
                    public void visitEnd() {
                        ...
                        for (AnnotationNode pad : this.visibleAnnotations) {
                            ...
                            for (int i2 = 0; i2 < pad.values.size(); i2 += 2) {
                                boolean value = pad.values.get(i2 + 1).equals(Boolean.TRUE);
                                if ("before".equals(pad.values.get(i2))) {
                                    before = value;
                                    continue;
                                }
                                after = value;
                        ...
                        if (before) {
                            int count;
                            for (int i3 = count = CacheUtil.getCacheLineSize() / byteLength - 1; i3 >= 1; --i3) {
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
            ...
            return new MethodNode(access, name, desc, signature, exceptions){
                 ...
                 */
                public void visitMaxs(int a2, int b2) {
                    try {
                        is_currently_computing_frames = true;
                        super.visitMaxs(a2, b2);
                    }
                    finally {
                ...
                private Frame<BasicValue>[] analyse() throws AnalyzerException {
                    Analyzer a2 = new Analyzer((Interpreter)new SimpleVerifier());
                    a2.analyze(TransformationAdapter.this.className, (MethodNode)this);
                    return a2.getFrames();
                }
                ...
                private void transformMethod(Frame<BasicValue>[] frames) {
                    ...
                    for (i2 = 0; i2 < frames.length; ++i2) {
                        frameMap.put(instructions.get(i2), frames[i2]);
                    }
                    block6: for (i2 = 0; i2 < instructions.size(); ++i2) {
                        AbstractInsnNode instruction = instructions.get(i2);
                        switch (instruction.getType()) {
                            case 2: {
                                ...
                                i2 = MappedObjectTransformer.transformArrayAccess(instructions, i2, frameMap, varInsn, mappedSubtype, varInsn.var);
                                continue block6;
                            }
                            case 4: {
                                ...
                                i2 = MappedObjectTransformer.replace(instructions, i2, instruction, list);
                                continue block6;
                            }
                            case 5: {
