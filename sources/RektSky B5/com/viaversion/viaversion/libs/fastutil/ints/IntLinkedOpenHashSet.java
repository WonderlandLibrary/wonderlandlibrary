...
Hash {
    ...
    public static IntLinkedOpenHashSet of(int ... a2) {
        IntLinkedOpenHashSet result = new IntLinkedOpenHashSet(a2.length, 0.75f);
        for (int element : a2) {
    ...
    public IntLinkedOpenHashSet clone() {
        IntLinkedOpenHashSet c2;
        try {
            c2 = (IntLinkedOpenHashSet)super.clone();
        }
    ...
    implements IntListIterator {
        ...
        public void remove() {
            ...
            if (this.prev == -1) {
                ...
                int n2 = this.prev;
                IntLinkedOpenHashSet.this.link[n2] = IntLinkedOpenHashSet.this.link[n2] ^ (IntLinkedOpenHashSet.this.link[this.prev] ^ (long)this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL;
            }
            if (this.next == -1) {
                IntLinkedOpenHashSet.this.last = this.prev;
            } else {
                int n3 = this.next;
                IntLinkedOpenHashSet.this.link[n3] = IntLinkedOpenHashSet.this.link[n3] ^ (IntLinkedOpenHashSet.this.link[this.next] ^ ((long)this.prev & 0xFFFFFFFFL) << 32) & 0xFFFFFFFF00000000L;
            }
            int pos = this.curr;
            this.curr = -1;
