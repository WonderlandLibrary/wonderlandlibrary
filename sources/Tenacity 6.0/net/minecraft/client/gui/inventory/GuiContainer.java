...
public abstract class GuiContainer extends GuiScreen
{
    ...
    public void initGui() {
        super.initGui();
        this.mc2.thePlayer.openContainer = this.inventorySlots;
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        if (ChestStealer.canSteal()) {
            if (ChestStealer.silent.isEnabled()) {
                ...
                GuiContainer.mc.inGameHasFocus = true;
                GuiContainer.mc.mouseHelper.grabMouseCursor();
                GuiContainer.mc.currentScreen = null;
                return;
            }
            if (ChestStealer.freeLook.isEnabled()) {
                GuiContainer.mc.inGameHasFocus = true;
                GuiContainer.mc.mouseHelper.grabMouseCursor();
            }
        }
        ...
        for (int i2 = 0; i2 < this.inventorySlots.inventorySlots.size(); ++i2) {
            ...
            if (this.isMouseOverSlot(slot, mouseX, mouseY) && slot.canBeHovered()) {
                ...
                Gui.drawGradientRect(j2, k2, j2 + 16, k2 + 16, -2130706433, -2130706433);
        ...
        RenderHelper.enableGUIStandardItemLighting();
        final InventoryPlayer inventoryplayer = this.mc2.thePlayer.inventory;
    ...
    private void drawSlot(final Slot slotIn) {
        ...
        final ItemStack itemstack2 = this.mc2.thePlayer.inventory.getItemStack();
        String s = null;
        ...
        if (itemstack == null) {
            ...
            if (s2 != null) {
                final TextureAtlasSprite textureatlassprite = this.mc2.getTextureMapBlocks().getAtlasSprite(s2);
                GlStateManager.disableLighting();
                this.mc2.getTextureManager().bindTexture(TextureMap.locationBlocksTexture);
    ...
    private void updateDragSplitting() {
        final ItemStack itemstack = this.mc2.thePlayer.inventory.getItemStack();
    ...
    protected void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) throws IOException {
        if (mouseButton - 100 == Minecraft.getMinecraft().gameSettings.keyBindInventory.getKeyCode()) {
            Minecraft.getMinecraft().thePlayer.closeScreen();
            return;
        }
        super.mouseClicked(mouseX, mouseY, mouseButton);
        final boolean flag = mouseButton == this.mc2.gameSettings.keyBindPickBlock.getKeyCode() + 100;
        ...
        if (mouseButton == 0 || mouseButton == 1 || flag) {
            ...
            if (this.mc2.gameSettings.touchscreen && flag2 && this.mc2.thePlayer.inventory.getItemStack() == null) {
                this.mc2.displayGuiScreen(null);
                return;
            }
            if (l != -1) {
                if (this.mc2.gameSettings.touchscreen) {
                    ...
                    if (this.mc2.thePlayer.inventory.getItemStack() == null) {
                        if (mouseButton == this.mc2.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                        ...
                        else if (mouseButton == this.mc2.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
    ...
    protected void mouseClickMove(final int mouseX, final int mouseY, final int clickedMouseButton, final long timeSinceLastClick) {
        final Slot slot = this.getSlotAtPosition(mouseX, mouseY);
        final ItemStack itemstack = this.mc2.thePlayer.inventory.getItemStack();
        if (this.clickedSlot != null && this.mc2.gameSettings.touchscreen) {
    ...
    protected void mouseReleased(final int mouseX, final int mouseY, final int state) {
        ...
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(null, slot)) {
            if (isShiftKeyDown()) {
                if (slot != null && slot.inventory != null && this.shiftClickedSlot != null) {
                    for (final Slot slot2 : this.inventorySlots.inventorySlots) {
                        if (slot2 != null && slot2.canTakeStack(this.mc2.thePlayer) && slot2.getHasStack() && slot2.inventory == slot.inventory && Container.canAddItemToSlot(slot2, this.shiftClickedSlot, true)) {
            ...
            else if (this.mc2.thePlayer.inventory.getItemStack() != null) {
                if (state == this.mc2.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
    ...
    protected void handleMouseClick(final Slot slotIn, int slotId, final int clickedButton, final int clickType) {
        ...
        this.mc2.playerController.windowClick(this.inventorySlots.windowId, slotId, clickedButton, clickType, this.mc2.thePlayer);
    }
    
    @Override
    protected void keyTyped(final char typedChar, final int keyCode) throws IOException {
        if (keyCode == 1 || keyCode == this.mc2.gameSettings.keyBindInventory.getKeyCode()) {
            this.mc2.thePlayer.closeScreen();
        }
        this.checkHotbarKeys(keyCode);
        if (this.theSlot != null && this.theSlot.getHasStack()) {
            if (keyCode == this.mc2.gameSettings.keyBindPickBlock.getKeyCode()) {
                this.handleMouseClick(this.theSlot, this.theSlot.slotNumber, 0, 3);
            }
            else if (keyCode == this.mc2.gameSettings.keyBindDrop.getKeyCode()) {
    ...
    protected boolean checkHotbarKeys(final int keyCode) {
        if (this.mc2.thePlayer.inventory.getItemStack() == null && this.theSlot != null) {
            for (int i = 0; i < 9; ++i) {
                if (keyCode == this.mc2.gameSettings.keyBindsHotbar[i].getKeyCode()) {
    ...
    public void onGuiClosed() {
        if (this.mc2.thePlayer != null) {
            this.inventorySlots.onContainerClosed(this.mc2.thePlayer);
        }
    }
    ...
    public void updateScreen() {
        super.updateScreen();
        if (!this.mc2.thePlayer.isEntityAlive() || this.mc2.thePlayer.isDead) {
            this.mc2.thePlayer.closeScreen();
        }
    }
