...
implements ViaVersionConfig {
    ...
    private BlockedProtocolVersions loadBlockedProtocolVersions() {
        ...
        int lowerBound = -1;
        int upperBound = -1;
        for (String s2 : this.getStringList("block-versions")) {
            ProtocolVersion protocolVersion;
            if (s2.isEmpty()) continue;
            char c2 = s2.charAt(0);
            if (c2 == '<' || c2 == '>') {
                protocolVersion = this.protocolVersion(s2.substring(1));
            ...
            protocolVersion = this.protocolVersion(s2);
    ...
    private @Nullable ProtocolVersion protocolVersion(String s2) {
        ProtocolVersion protocolVersion = ProtocolVersion.getClosest(s2);
        if (protocolVersion == null) {
            Via.getPlatform().getLogger().warning("Unknown protocol version in block-versions: " + s2);
            return null;
        }
