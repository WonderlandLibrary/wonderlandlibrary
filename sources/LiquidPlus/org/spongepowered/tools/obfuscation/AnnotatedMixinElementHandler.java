...
abstract class AnnotatedMixinElementHandler {
    ...
    protected final void checkConstraints(ExecutableElement method, AnnotationHandle annotation2) {
        try {
            ConstraintParser.Constraint constraint = ConstraintParser.parse((String)annotation2.getValue("constraints"));
            try {
                constraint.check(this.ap.getTokenProvider());
            }
            catch (ConstraintViolationException ex) {
                this.ap.printMessage(Diagnostic.Kind.ERROR, ex.getMessage(), method, annotation2.asMirror());
            }
        }
        catch (InvalidConstraintException ex) {
            this.ap.printMessage(Diagnostic.Kind.WARNING, ex.getMessage(), method, annotation2.asMirror());
        }
    }

    protected final void validateTarget(Element element, AnnotationHandle annotation2, AliasedElementName name, String type) {
        if (element instanceof ExecutableElement) {
            this.validateTargetMethod((ExecutableElement)element, annotation2, name, type, false, false);
        } else if (element instanceof VariableElement) {
            this.validateTargetField((VariableElement)element, annotation2, name, type);
        }
    }

    protected final void validateTargetMethod(ExecutableElement method, AnnotationHandle annotation2, AliasedElementName name, String type, boolean overwrite, boolean merge) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            if (targetMethod != null) {
                ...
                this.validateMethodVisibility(method, annotation2, type, target, targetMethod);
                continue;
            }
            if (merge) continue;
            this.printMessage(Diagnostic.Kind.WARNING, "Cannot find target for " + type + " method in " + target, method, annotation2);
        }
    }

    private void validateMethodVisibility(ExecutableElement method, AnnotationHandle annotation2, String type, TypeHandle target, MethodHandle targetMethod) {
        ...
        if (visTarget.ordinal() > visMethod.ordinal()) {
            this.printMessage(Diagnostic.Kind.WARNING, (Object)((Object)visMethod) + " " + type + " method cannot reduce " + visibility, method, annotation2);
        } else if (visTarget == Visibility.PRIVATE && visMethod.ordinal() > visTarget.ordinal()) {
            this.printMessage(Diagnostic.Kind.WARNING, (Object)((Object)visMethod) + " " + type + " method will upgrade " + visibility, method, annotation2);
        }
    }

    protected final void validateTargetField(VariableElement field, AnnotationHandle annotation2, AliasedElementName name, String type) {
        ...
        for (TypeHandle target : this.mixin.getTargets()) {
            ...
            this.ap.printMessage(Diagnostic.Kind.WARNING, "Cannot find target for " + type + " field in " + target, field, annotation2.asMirror());
        }
    }
    ...
    private void printMessage(Diagnostic.Kind kind, String msg, Element e, AnnotationHandle annotation2) {
        if (annotation2 == null) {
            this.ap.printMessage(kind, msg, e);
        } else {
            this.ap.printMessage(kind, msg, e, annotation2.asMirror());
        }
    }
    ...
    static class AliasedElementName {
        ...
        public AliasedElementName(Element element, AnnotationHandle annotation2) {
            this.originalName = element.getSimpleName().toString();
            this.aliases = annotation2.getList("aliases");
        }
    ...
    static abstract class AnnotatedElement<E extends Element> {
        ...
        public AnnotatedElement(E element, AnnotationHandle annotation2) {
