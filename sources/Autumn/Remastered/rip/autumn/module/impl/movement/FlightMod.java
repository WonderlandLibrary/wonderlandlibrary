...
public final class FlightMod extends Module {
   ...
   public final BoolOption viewBobbing, multiplier;
   public final DoubleOption multiplyspeed, multiplytime, speed, vanillaSpeed;
   ...
   public void onSendPacket(SendPacketEvent event) {
      switch (mode.getValue().toString()) {
         ...
         case "MINEPLEX":
            ...
            break;
         case "DISABLER":
            if(event.getPacket() instanceof C03PacketPlayer && !this.tp)
               event.setCancelled();
            break;
      }
   }
   ...
   public void onMove(MoveEvent event) {
      ...
      case WATCHDOG:
         if (player.isMoving()) {
            ...
            MovementUtils.setSpeed(Math.max(this.moveSpeed, MovementUtils.getBaseMoveSpeed()));
            ++this.stage;
         }
         break;
      case MINEPLEX:
         ...
         break;
      case VANILLA:
         MovementUtils.setSpeed((double)((Double)this.vanillaSpeed.getValue()).intValue());
         break;
      case DISABLER:
      ...
      case CUBECRAFT:
         ...
         if (this.ticks > 12) {
            if (this.stage == 4) {
               MovementUtils.setSpeed(.953532D);
               this.stage = 0;
            } else {
               MovementUtils.setSpeed(0.121984218421847D);
            }
         } else {
            MovementUtils.setSpeed(0.0D);
         }
   ...
   public void onMotionUpdate(MotionUpdateEvent event) {
      ...
      case "WATCHDOG":
         ...
         if (event.isPre()) {
            ...
            if (this.stage > 2) {
               ...
               event.setPosY(mc.thePlayer.posY + this.y);
            }
      ...
      case "MINEPLEX":
         ...
         if (stopwatch.elapsed(6000L) && !this.tp) {
            ...
            netHandler.addToSendQueueSilent(new C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, true));
      ...
      case "CUBECRAFT":
         if (event.isPre()) {
            ...
            ++this.ticks;
            float y = (float)Math.floor(mc.thePlayer.posY);
            if (this.ticks == 1 && !mc.thePlayer.onGround) {
               this.toggle();
            }
      ...
      if (event.isPre()) {
         offset = mc.thePlayer.posX - mc.thePlayer.prevPosX;
