...
implements GLUtessellator {
    ...
    public GLUtessellatorImpl() {
        ...
        for (int i2 = 0; i2 < this.cache.length; ++i2) {
            this.cache[i2] = new CachedVertex();
        }
    }
    ...
    public void gluTessNormal(double x2, double y2, double z2) {
        this.normal[0] = x2;
        this.normal[1] = y2;
        this.normal[2] = z2;
    }
    ...
    private boolean addVertex(double[] coords, Object vertexData) {
        ...
        if (e2 == null) {
            e2 = Mesh.__gl_meshMakeEdge(this.mesh);
            if (e2 == null) {
                return false;
            }
            if (!Mesh.__gl_meshSplice(e2, e2.Sym)) {
                return false;
            }
        } else {
        ...
        e2.Org.data = vertexData;
        e2.Org.coords[0] = coords[0];
        e2.Org.coords[1] = coords[1];
        e2.Org.coords[2] = coords[2];
        e2.winding = 1;
        e2.Sym.winding = -1;
        this.lastEdge = e2;
        return true;
    ...
    private void cacheVertex(double[] coords, Object vertexData) {
        ...
        CachedVertex v2 = this.cache[this.cacheCount];
        v2.data = vertexData;
        v2.coords[0] = coords[0];
        v2.coords[1] = coords[1];
        v2.coords[2] = coords[2];
        ++this.cacheCount;
    }

    private boolean flushCache() {
        CachedVertex[] v2 = this.cache;
        this.mesh = Mesh.__gl_meshNewMesh();
        if (this.mesh == null) {
            return false;
        }
        for (int i2 = 0; i2 < this.cacheCount; ++i2) {
            CachedVertex vertex = v2[i2];
    ...
    public void gluTessVertex(double[] coords, int coords_offset, Object vertexData) {
        ...
        for (int i2 = 0; i2 < 3; ++i2) {
            double x2 = coords[i2 + coords_offset];
            if (x2 < -1.0E150) {
                x2 = -1.0E150;
                tooLarge = true;
            }
    ...
    void callBeginOrBeginData(int a2) {
        if (this.callBeginData != NULL_CB) {
            this.callBeginData.beginData(a2, this.polygonData);
        } else {
            this.callBegin.begin(a2);
        }
    }

    void callVertexOrVertexData(Object a2) {
        if (this.callVertexData != NULL_CB) {
            this.callVertexData.vertexData(a2, this.polygonData);
        } else {
            this.callVertex.vertex(a2);
        }
    }

    void callEdgeFlagOrEdgeFlagData(boolean a2) {
        if (this.callEdgeFlagData != NULL_CB) {
            this.callEdgeFlagData.edgeFlagData(a2, this.polygonData);
        } else {
            this.callEdgeFlag.edgeFlag(a2);
        }
    }
    ...
    void callErrorOrErrorData(int a2) {
        if (this.callErrorData != NULL_CB) {
            this.callErrorData.errorData(a2, this.polygonData);
        } else {
            this.callError.error(a2);
        }
    }
}
