...
public class XmlConfiguration extends AbstractConfiguration implements Reconfigurable
{
    ...
    public XmlConfiguration(final LoggerContext loggerContext, final ConfigurationSource configSource) {
        ...
        try {
            ...
            for (final Map.Entry<String, String> entry : attrs.entrySet()) {
                ...
                final String value = this.getConfigurationStrSubstitutor().replace(entry.getValue());
        ...
        catch (final SAXException | IOException | ParserConfigurationException e2) {
    ...
    private static void setFeature(final DocumentBuilderFactory factory, final String featureName, final boolean value) {
        try {
            ...
            XmlConfiguration.LOGGER.warn("The DocumentBuilderFactory [{}] does not support the feature [{}]: {}", factory, featureName, e);
        }
        catch (final AbstractMethodError err) {
            XmlConfiguration.LOGGER.warn("The DocumentBuilderFactory [{}] is out of date and does not support setFeature: {}", factory, err);
        }
    }
