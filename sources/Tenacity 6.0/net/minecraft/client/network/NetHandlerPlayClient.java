...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleSpawnObject(final S0EPacketSpawnObject packetIn) {
        ...
        if (packetIn.getType() == 10) {
            entity = EntityMinecart.getMinecart(this.clientWorldController, d0, d2, d3, EntityMinecart.EnumMinecartType.byNetworkID(packetIn.func_149009_m()));
        }
    ...
    @Override
    public void handlePlayerPosLookHypixel(final S08PacketPlayerPosLook packetIn) {
        final EntityPlayer entityplayer = this.gameController.thePlayer;
        double d0 = packetIn.getX();
        double d2 = packetIn.getY();
        double d3 = packetIn.getZ();
    ...
    public void handleChat(final S02PacketChat packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
        final ChatReceivedEvent e = new ChatReceivedEvent(packetIn.getType(), packetIn.getChatComponent());
        Tenacity.INSTANCE.getEventProtocol().handleEvent(e);
        if (e.isCancelled() || e.message == null) {
            return;
        }
    ...
    public void handlePlayerListItem(final S38PacketPlayerListItem packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
        for (final S38PacketPlayerListItem.AddPlayerData s38packetplayerlistitem$addplayerdata : packetIn.getEntries()) {
    ...
    public void handleSoundEffect(final S29PacketSoundEffect packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
        if (Flight.hiddenBlocks.contains(new BlockPos(packetIn.getX() - 0.5, packetIn.getY() + 0.5, packetIn.getZ() - 0.5))) {
            return;
        }
    ...
    public void handleResourcePack(final S48PacketResourcePackSend packetIn) {
        if (!Enhancements.validateResourcePackUrl(this, packetIn)) {
            return;
        }
        ...
        if (s.startsWith("level://")) {
            ...
            final Minecraft gameController;
            final GuiScreen guiScreenIn;
            final String s4;
            final String s5;
            this.gameController.addScheduledTask(() -> {
                gameController = this.gameController;
                new GuiYesNo(new GuiYesNoCallback() {
                    ...
                    public void confirmClicked(final boolean result, final int id) {
                        ...
                        if (result) {
                            ...
                            NetHandlerPlayClient.this.netManager.sendPacket(new C19PacketResourcePackStatus(this.val$s1, C19PacketResourcePackStatus.Action.ACCEPTED));
                            Futures.addCallback((ListenableFuture)NetHandlerPlayClient.this.gameController.getResourcePackRepository().downloadResourcePack(this.val$s, this.val$s1), (FutureCallback)new FutureCallback<Object>() {
                                public void onSuccess(final Object p_onSuccess_1_) {
                                    NetHandlerPlayClient.this.netManager.sendPacket(new C19PacketResourcePackStatus(GuiYesNoCallback.this.val$s1, C19PacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
                                }
                                
                                public void onFailure(final Throwable p_onFailure_1_) {
                                    NetHandlerPlayClient.this.netManager.sendPacket(new C19PacketResourcePackStatus(GuiYesNoCallback.this.val$s1, C19PacketResourcePackStatus.Action.FAILED_DOWNLOAD));
                                }
                            }, MoreExecutors.directExecutor());
                        }
                        else {
                            ...
                            NetHandlerPlayClient.this.netManager.sendPacket(new C19PacketResourcePackStatus(this.val$s1, C19PacketResourcePackStatus.Action.DECLINED));
                        }
                ...
                gameController.displayGuiScreen(guiScreenIn);
            });
        }
    }
